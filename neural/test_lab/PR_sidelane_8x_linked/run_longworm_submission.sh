...
export ARM_ONLY="${ARM_ONLY:-35_v1_5_longworm_context_mech_l11_d528_h12_kv4_non_ngram_brotli}"
export TRACK_4K_BPB="${TRACK_4K_BPB:-0}"
export LEADERBOARD_METRIC_PREF="${LEADERBOARD_METRIC_PREF:-submission}"

# Keep Longworm isolated from Rascal runners unless explicitly overridden.
if [[ -z "${TRAIN_PY:-}" ]]; then
  export TRAIN_PY="${SCRIPT_DIR}/../../experiments/Longworm/train_longworm.py"
fi

echo "============================================================"
echo "Longworm submission launcher"
