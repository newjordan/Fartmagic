#!/usr/bin/env bash
# Bench the whale dkdv inline-delta kernel before/after the
# SeparateMaskingIterations patch on the four shapes in tracked_env.sh.
# This script does NOT modify vault/whale_kernel_triton.py — it must be
# patched by hand per vault_patch.md before the AFTER pass.
#
# Sequencing: do NOT run the AFTER pass until the sibling leg
# legs/2026-04-16_whale_dkdv_early_exit/ has shown a positive bench delta
# on its base-kernel patch.
#
# Usage:
#   bash legs/2026-04-16_whale_dkdv_early_exit_inline_delta/run.sh before
#   # (apply vault_patch.md to vault/whale_kernel_triton.py)
#   bash legs/2026-04-16_whale_dkdv_early_exit_inline_delta/run.sh after
#
# Outputs land in evidence/ and logs/ under this leg directory.
set -euo pipefail

REPO_ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)"
LEG_DIR="${REPO_ROOT}/legs/2026-04-16_whale_dkdv_early_exit_inline_delta"
...
echo "[run] phase=${PHASE} ts=${TS}" | tee "${LOG_FILE}"
echo "[run] preflight"  | tee -a "${LOG_FILE}"
{
  ...
  echo "  WHALE_BWD_VARIANT: ${WHALE_BWD_VARIANT}"
  echo "  WHALE_FUSED_DELTA_T_MAX: ${WHALE_FUSED_DELTA_T_MAX}"
} | tee -a "${LOG_FILE}"
...
PYTHONPATH="${REPO_ROOT}" ${PY} \
  ...
  --out "${NUMERICS_OUT}" \
  --label "dkdv_early_exit_inline_delta_${PHASE}_${TS}" \
  2>&1 | tee -a "${LOG_FILE}"

# Per-shape backend bench. WHALE_FUSED_DELTA_T_MAX=8192 above forces every
# shape (including T=8192) into _attn_bwd_dkdv_inline_delta_kernel.
