...
# --- Fused RMSNorm + Up Projection + LeakyReLU² (Arm C megakernel) ---
# Avoids materializing the 1536-wide MLP intermediate in HBM.
# Forward: load x[M,D], compute norm in-register, matmul up_w, apply act, store [M,E].
# We fuse norm+activation around the matmul — the matmul itself uses torch for cuBLAS speed.

class FusedNormActMLP(torch.autograd.Function):
    """Fused RMSNorm → up_proj → LeakyReLU² → down_proj.
    Saves one HBM roundtrip by fusing norm before matmul and activation after."""

    @staticmethod
    def forward(ctx, x, up_w, down_w, ln_scale_factor):
        # Step 1: RMSNorm (fused with scale factor)
        x_norm = F.rms_norm(x, (x.size(-1),)) * ln_scale_factor

        # Step 2: up projection
        h = F.linear(x_norm, up_w.to(x.dtype))

        # Step 3: LeakyReLU² activation in-place where possible
        a = torch.where(h >= 0, h, 0.5 * h)
        act_out = a * a

        # Step 4: down projection
        out = F.linear(act_out, down_w.to(x.dtype))

        # Save for backward
        ctx.save_for_backward(x, x_norm, up_w, down_w, h)
        ctx.ln_scale_factor = ln_scale_factor
        return out

    @staticmethod
    def backward(ctx, grad_out):
        x, x_norm, up_w, down_w, h = ctx.saved_tensors

        # Backward through down_proj
        grad_act = F.linear(grad_out, down_w.t().to(grad_out.dtype))
        grad_down_w = grad_out.reshape(-1, grad_out.size(-1)).t() @ \
            torch.where(h >= 0, h, 0.5 * h).square().reshape(-1, h.size(-1)).to(grad_out.dtype)

        # Backward through LeakyReLU²
        a = torch.where(h >= 0, h, 0.5 * h)
        slope = torch.where(h >= 0, h.new_ones(()), h.new_full((), 0.5))
        grad_h = grad_act * (2.0 * a * slope)

        # Backward through up_proj
        grad_x_norm = F.linear(grad_h, up_w.t().to(grad_h.dtype))
        grad_up_w = grad_h.reshape(-1, grad_h.size(-1)).t() @ x_norm.reshape(-1, x_norm.size(-1)).to(grad_h.dtype)

        # Backward through RMSNorm (use PyTorch autograd for correctness)
        # We need grad w.r.t. x given x_norm = rms_norm(x) * scale
        grad_x_norm_unscaled = grad_x_norm * ctx.ln_scale_factor
        # RMSNorm backward: standard formula
        D = x.size(-1)
        rms = (x.float().pow(2).mean(dim=-1, keepdim=True) + 1e-6).rsqrt()
        grad_x = (grad_x_norm_unscaled.float() * rms -
                  x.float() * rms.pow(3) * (x.float() * grad_x_norm_unscaled.float()).mean(dim=-1, keepdim=True))
        grad_x = grad_x.to(x.dtype)

        return grad_x, grad_up_w.to(up_w.dtype), grad_down_w.to(down_w.dtype), None
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.fused_norm_act = self.kernel_mode == "fused_norm_act"
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor, ln_scale_factor: float = 1.0) -> Tensor:
        if self.fused_norm_act:
            return FusedNormActMLP.apply(x, up_w, down_w, ln_scale_factor)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.mlp.fused_norm_act:
            # Fused mode: MLP handles norm+up+act+down internally
            x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(x_out, up_w, down_w, self.ln_scale_factor)
        else:
