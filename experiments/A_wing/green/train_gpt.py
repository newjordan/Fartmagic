        ...
        if args.ngram_eval_order >= 2:
            ...
            if rank == 0:
                ...
                if ng_coverage >= 0.999999:
                    log0(
                        f"final_int5_sliding_window_ngram{args.ngram_eval_order} val_loss:{ng_loss:.4f} "
                        f"val_bpb:{ng_bpb:.4f} eval_time:{ng_eval_ms:.0f}ms"
                    )
                    log0(
                        f"final_int5_sliding_window_ngram{args.ngram_eval_order}_exact "
                        f"val_loss:{ng_loss:.8f} val_bpb:{ng_bpb:.8f}"
                    )
                else:
                    log0(
                        f"final_int5_sliding_window_ngram{args.ngram_eval_order}_partial val_loss:{ng_loss:.4f} "
                        f"val_bpb:{ng_bpb:.4f} coverage:{ng_coverage:.4f} eval_time:{ng_eval_ms:.0f}ms"
                    )
                    log0(
                        f"final_int5_sliding_window_ngram{args.ngram_eval_order}_partial_exact "
