...
class Hyperparameters:
    ...
    # FlatServe: modify flat layers to serve the crawler
    flat_residual_scale = float(os.environ.get("FLAT_RESIDUAL_SCALE", "1.0"))  # <1.0 = smaller flat updates, smoother for crawler
    flat_noise_std = float(os.environ.get("FLAT_NOISE_STD", "0.0"))            # >0 = inject noise for crawler to denoise
    flat_skip_to_crawl = bool(int(os.environ.get("FLAT_SKIP_TO_CRAWL", "0")))  # route U-Net skips through crawler
...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        # FlatServe defaults (overridden after construction via env vars)
        self._flat_residual_scale: float = 1.0
        self._flat_noise_std: float = 0.0
        self._flat_skip_to_crawl: bool = False
        self.final_norm = RMSNorm()
        self.lm_head = None if tie_embeddings else CastedLinear(model_dim, vocab_size, bias=False)
        if self.lm_head is not None:
            self.lm_head._zero_init = True
    ...
    def _flat_post(self, x: Tensor, x_pre: Tensor) -> Tensor:
        """Apply FlatServe modifications to flat layer output."""
        # Residual scaling: x = x_pre + scale * (x - x_pre)
        if self._flat_residual_scale < 1.0:
            delta = x - x_pre
            x = x_pre + self._flat_residual_scale * delta
        # Noise injection (training only): force crawler to learn denoising
        if self._flat_noise_std > 0.0 and self.training:
            x = x + self._flat_noise_std * torch.randn_like(x)
        return x
    ...
    def _run_helix(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict) -> Tensor:
        ...
        for i in range(self.flat_decoder_layers):
            bi = self.flat_encoder_layers + i
            if self._flat_skip_to_crawl and skips:
                # Route skip through crawler instead of direct inject
                skip_signal = self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
                x_crawl = x_crawl + skip_signal
            elif skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x_pre = x
            x = self.flat_blocks[bi](x, x0)
...
def main() -> None:
    ...
    # FlatServe: configure flat layer modifications
    if hasattr(base_model, '_flat_residual_scale'):
        base_model._flat_residual_scale = args.flat_residual_scale
        base_model._flat_noise_std = args.flat_noise_std
        base_model._flat_skip_to_crawl = args.flat_skip_to_crawl
        if args.flat_residual_scale < 1.0 or args.flat_noise_std > 0.0 or args.flat_skip_to_crawl:
            log0(f"flatserve:residual_scale={args.flat_residual_scale} noise_std={args.flat_noise_std} skip_to_crawl={args.flat_skip_to_crawl}")
