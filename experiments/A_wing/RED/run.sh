#!/bin/bash
set -euo pipefail
# A-WING RED: Mixer-first, startup-bounded variant.
...
echo "============================================"
echo "  A-WING RED — Learned Mixer Head (Fast Prefill)"
echo "  Seed: ${SEED}"
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
    ...
    2>&1 | tee "logs/awing_red_mixer_s${SEED}_$(date +%Y%m%d_%H%M%S).log"
