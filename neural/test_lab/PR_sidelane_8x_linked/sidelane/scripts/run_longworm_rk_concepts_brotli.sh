...
if [[ -z "${TRAIN_PY:-}" || ! -f "${TRAIN_PY}" ]]; then
  echo "TRAIN_PY not found. Expected dedicated Longworm runner at:" >&2
  echo "  ${PG_LAB_ROOT}/experiments/Longworm/train_longworm.py" >&2
  echo "Set TRAIN_PY=/abs/path/to/experiments/Longworm/train_longworm.py" >&2
  exit 1
fi
...
RUNS_SUBDIR="${RUNS_SUBDIR:-${PROJECT_CODENAME}_rk_concepts_brotli}"
...
ARMS=(
  "41_longworm_k2_non_ngram_brotli|candidate"
  "42_longworm_k4_non_ngram_brotli|candidate"
  "43_longworm_k2_4_hybrid_non_ngram_brotli|candidate"
)
...
echo "Longworm RK concepts sweep (k2 / k4 / k2_4_hybrid, brotli)"
...
for arm_spec in "${ARMS[@]}"; do
  ...
  if (
    ...
    export RUN_ID="${PROJECT_CODENAME}_rk_concepts_brotli_04_longctx_${arm_name}_s${SEED}_${TS}"
  ...
  printf "%s_rk_concepts_brotli\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
...
echo
echo "${PROJECT_CODENAME}_rk_concepts_sweep_complete"
