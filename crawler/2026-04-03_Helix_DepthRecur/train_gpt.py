...
class Hyperparameters:
    ...
    # Depth recurrence: specific flat layers fire twice (field standard, PR #1204/#1260)
    recur_layers = tuple(int(x) for x in os.environ.get("RECUR_LAYERS", "").split(",") if x.strip()) if os.environ.get("RECUR_LAYERS", "").strip() else ()
    recur_start_step = int(os.environ.get("RECUR_START_STEP", "0"))  # 0 = always on, >0 = delayed start
...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        # Depth recurrence state (toggled by training loop based on step count)
        self._recur_layers_set: set[int] = set()
        self._recur_active: bool = False
        self.final_norm = RMSNorm()
        self.lm_head = None if tie_embeddings else CastedLinear(model_dim, vocab_size, bias=False)
        if self.lm_head is not None:
            self.lm_head._zero_init = True
    ...
    def _run_encoder(self, x: Tensor, x0: Tensor) -> tuple[Tensor, list[Tensor]]:
        ...
        for i in range(self.flat_encoder_layers):
            ...
            if self._recur_active and i in self._recur_layers_set:
                x = self.flat_blocks[i](x, x0)  # depth recurrence: fire again
            skips.append(x)
        return x, skips
    ...
    def _run_decoder(self, x: Tensor, x0: Tensor, skips: list[Tensor]) -> Tensor:
        for i in range(self.flat_decoder_layers):
            ...
            if self._recur_active and bi in self._recur_layers_set:
                x = self.flat_blocks[bi](x, x0)  # depth recurrence: fire again
        return x
...
def main() -> None:
    ...
    # Depth recurrence: configure which flat layers fire twice
    if hasattr(base_model, '_recur_layers_set') and args.recur_layers:
        base_model._recur_layers_set = set(args.recur_layers)
        base_model._recur_active = (args.recur_start_step == 0)
        log0(f"depth_recurrence:layers={args.recur_layers} start_step={args.recur_start_step} active={base_model._recur_active}")
    ...
    while True:
        ...
        zero_grad_all()
        step += 1
        # Depth recurrence: delayed start activation
        if (args.recur_layers and args.recur_start_step > 0
                and step == args.recur_start_step and not base_model._recur_active):
            base_model._recur_active = True
            log0(f"depth_recurrence:activated at step {step}")
