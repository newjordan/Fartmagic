...
def _spec_hsl_color(spec: dict, role: str, fallback_h: float) -> tuple[float, float, float]:
    palette = spec.get("palette", {}) if isinstance(spec, dict) else {}
    raw = palette.get(role)
    if isinstance(raw, list) and len(raw) >= 3:
        h = _safe_float(raw[0], fallback_h)
        s = _safe_float(raw[1], 0.72)
        l = _safe_float(raw[2], 0.52)
        return _hsl_rgb(h, s, l)
    return _hsl_rgb(fallback_h, 0.72, 0.52)


def _build_midnight_spec_points(spec: dict, *, step_norm: float) -> tuple[list[dict], list[dict]]:
    facts = spec.get("facts", {}) if isinstance(spec, dict) else {}
    layout = spec.get("layout", {}) if isinstance(spec, dict) else {}
    quant = facts.get("quant_bits", {}) if isinstance(facts, dict) else {}

    layers = max(1, _safe_int(facts.get("layers"), 12))
    q_heads = max(1, _safe_int(facts.get("query_heads"), 8))
    kv_heads = max(1, _safe_int(facts.get("kv_heads"), 4))
    gqa_ratio = _safe_float(facts.get("gqa_ratio"), q_heads / max(1.0, float(kv_heads)))
    rope_dims = max(0, _safe_int(facts.get("rope_dims"), 16))
    bigram_dim = max(0, _safe_int(facts.get("bigram_dim"), 2048))
    xsa_last_n = max(0, _safe_int(facts.get("xsa_last_n"), 11))
    attn_bits = max(0, _safe_int(quant.get("attn"), 5))
    mlp_bits = max(0, _safe_int(quant.get("mlp"), 6))
    aux_bits = max(0, _safe_int(quant.get("aux"), 6))
    embed_bits = max(0, _safe_int(quant.get("embed"), 8))
    other_bits = max(0, _safe_int(quant.get("other"), 8))

    base_y = _safe_float(layout.get("base_y"), 1.70)
    layer_spacing = max(0.10, _safe_float(layout.get("layer_spacing"), 0.74))
    bigram_y = _safe_float(layout.get("bigram_y"), 0.00)
    embed_y = _safe_float(layout.get("embed_y"), 0.72)
    artifact_y = _safe_float(layout.get("artifact_y"), 12.25)
    q_radius = max(0.5, _safe_float(layout.get("head_q_radius"), 4.18))
    kv_radius = max(0.5, _safe_float(layout.get("head_kv_radius"), 3.24))
    bar_point_spacing = max(0.03, _safe_float(layout.get("bar_point_spacing"), 0.12))
    bar_height_scale = max(0.05, _safe_float(layout.get("bar_height_scale"), 0.34))
    ...
    def add_point(
        *,
        ...
        block: int = -1,
        bitwidth: int = 0,
        xsa_active: bool = False,
        quant_name: str = "",
        q_heads_v: int = 0,
        kv_heads_v: int = 0,
        head_group: int = -1,
    ) -> int:
        stage_v = (stage or role).lower()
        fallback_h = STAGE_HUE.get(stage_v, STAGE_HUE.get(role, 0.58))
        r, g, b = _spec_hsl_color(spec, role, fallback_h)
        p = {
            "x": float(x),
            "y": float(y),
            "z": float(z),
            ...
            "gqa_ratio": float(gqa_ratio),
            "q_heads": int(q_heads_v),
            "kv_heads": int(kv_heads_v),
            "rope_dims": int(rope_dims),
            "bigram_dim": int(bigram_dim),
            "xsa_active": bool(xsa_active),
    ...
    def add_edge(i: int, j: int, *, flow_kind: str, magnitude: float = 0.7) -> None:
        flow = _clamp(magnitude, 0.05, 1.9)
        edges.append(
            {
                "i": int(i),
                "j": int(j),
                ...
                "token_count": 0,
                "flow_kind": flow_kind,
                "src_stage": str(points[i].get("stage", "")),
                "dst_stage": str(points[j].get("stage", "")),
                "src_key": f"{flow_kind}:src:{i}",
                "dst_key": f"{flow_kind}:dst:{j}",
                "magnitude": flow,
            }
        )

    def safe_phase(idx: int) -> float:
        return (0.19 * idx) % 1.0

    # Input basis anchors
    bigram_idx = add_point(
        role="bigram",
        x=MIDNIGHT_LANES["bigram"],
        y=bigram_y,
        z=0.0,
        label=f"bigram basis dim={bigram_dim}",
        stage="bigram",
        kind="bigram",
        layer_index=0,
    )
    rope_idx = add_point(
        role="rope",
        x=MIDNIGHT_LANES["rope"],
        y=embed_y * 0.45,
        z=0.0,
        label=f"rope basis dims={rope_dims}",
        stage="rope",
        kind="rope",
        layer_index=0,
    )
    embed_idx = add_point(
        role="embed",
        x=MIDNIGHT_LANES["embed"],
        y=embed_y,
        z=0.0,
        label=f"embedding int{embed_bits}",
        stage="embed",
        kind="embed",
        layer_index=0,
        bitwidth=embed_bits,
        quant_name=f"embed=int{embed_bits}",
    )
    add_edge(bigram_idx, embed_idx, flow_kind="basis_to_embed", magnitude=0.8)
    add_edge(rope_idx, embed_idx, flow_kind="rope_to_embed", magnitude=0.75)

    layer_nodes: list[int] = []
    for li in range(layers):
        y = base_y + li * layer_spacing
        z = 0.18 * math.sin(0.65 * li)
        xsa_active = li >= max(0, layers - xsa_last_n)
        layer_idx = add_point(
            role="layer",
            x=MIDNIGHT_LANES["layer"],
            y=y,
            z=z,
            label=f"layer {li + 1}/{layers} attn=int{attn_bits} mlp=int{mlp_bits}",
            stage="layer",
            kind="layer",
            layer_index=li,
            block=li,
            bitwidth=max(attn_bits, mlp_bits),
            xsa_active=xsa_active,
            quant_name=f"attn=int{attn_bits},mlp=int{mlp_bits}",
        )
        layer_nodes.append(layer_idx)
        if li == 0:
            add_edge(embed_idx, layer_idx, flow_kind="embed_to_layer", magnitude=0.9)
        else:
            add_edge(layer_nodes[li - 1], layer_idx, flow_kind="layer_stack", magnitude=0.95)

        # Quantization barcode columns
        mlp_idx = add_point(
            role="mlp",
            x=MIDNIGHT_LANES["mlp"] + bar_point_spacing * (li % 3),
            y=y,
            z=-0.20 + bar_height_scale * math.sin(0.8 * li),
            label=f"layer {li + 1} mlp int{mlp_bits}",
            stage="mlp",
            kind="mlp",
            layer_index=li,
            block=li,
            bitwidth=mlp_bits,
            quant_name=f"mlp=int{mlp_bits}",
        )
        aux_idx = add_point(
            role="aux",
            x=MIDNIGHT_LANES["aux"] + bar_point_spacing * ((li + 1) % 3),
            y=y,
            z=0.00 + bar_height_scale * math.cos(0.7 * li),
            label=f"layer {li + 1} aux int{aux_bits}",
            stage="aux",
            kind="aux",
            layer_index=li,
            block=li,
            bitwidth=aux_bits,
            quant_name=f"aux=int{aux_bits}",
        )
        other_idx = add_point(
            role="other",
            x=MIDNIGHT_LANES["other"] + bar_point_spacing * ((li + 2) % 3),
            y=y,
            z=0.20 + bar_height_scale * math.sin(0.55 * li + 0.4),
            label=f"layer {li + 1} other int{other_bits}",
            stage="other",
            kind="other",
            layer_index=li,
            block=li,
            bitwidth=other_bits,
            quant_name=f"other=int{other_bits}",
        )
        add_edge(layer_idx, mlp_idx, flow_kind="layer_to_mlp", magnitude=0.62)
        add_edge(layer_idx, aux_idx, flow_kind="layer_to_aux", magnitude=0.58)
        add_edge(layer_idx, other_idx, flow_kind="layer_to_other", magnitude=0.54)

        if xsa_active:
            xsa_idx = add_point(
                role="xsa",
                x=MIDNIGHT_LANES["xsa"],
                y=y,
                z=0.28 * math.sin(0.9 * li),
                label=f"layer {li + 1} xsa active",
                stage="xsa",
                kind="xsa",
                layer_index=li,
                block=li,
            ...
            add_edge(layer_idx, xsa_idx, flow_kind="layer_to_xsa", magnitude=0.66)

        # GQA ring: 8 query heads -> 4 KV heads (ratio 2.0)
        q_base = q_heads / 2.0
        kv_base = kv_heads / 2.0
        head_nodes: list[int] = []
        kv_nodes: list[int] = []
        for h in range(q_heads):
            qz = (h - (q_base - 0.5)) * 0.17
            qx = MIDNIGHT_LANES["attn_q"] + 0.25 * math.cos((2.0 * math.pi * h) / max(1, q_heads))
            qn = add_point(
                role="attn_q",
                x=qx,
                y=y,
                z=qz,
                label=f"layer {li + 1} q_head {h + 1}/{q_heads} int{attn_bits}",
                stage="head_q",
                kind="attn_q",
                layer_index=li,
                block=li,
                ...
                kv_heads_v=kv_heads,
                head_group=h,
                quant_name=f"attn=int{attn_bits}",
            )
            head_nodes.append(qn)
            add_edge(layer_idx, qn, flow_kind="layer_to_q", magnitude=0.52)
        for kv in range(kv_heads):
            kz = (kv - (kv_base - 0.5)) * 0.26
            kx = MIDNIGHT_LANES["attn_kv"] + 0.22 * math.cos((2.0 * math.pi * kv) / max(1, kv_heads))
            kn = add_point(
                role="attn_kv",
                x=kx,
                y=y,
                z=kz,
                label=f"layer {li + 1} kv_head {kv + 1}/{kv_heads} int{attn_bits}",
                stage="head_kv",
                kind="attn_kv",
                layer_index=li,
                block=li,
            ...
            kv_nodes.append(kn)
            add_edge(layer_idx, kn, flow_kind="layer_to_kv", magnitude=0.50)
        for h, qn in enumerate(head_nodes):
            kvn = kv_nodes[h % kv_heads]
            add_edge(qn, kvn, flow_kind="gqa_q_to_kv", magnitude=0.72)

    head_in_idx = add_point(
        role="head_in",
        x=MIDNIGHT_LANES["metric"] - 2.0,
        y=artifact_y - 0.35,
        z=0.0,
        label="head_in logits",
        stage="head_in",
        kind="head_in",
        layer_index=layers - 1,
    )
    add_edge(layer_nodes[-1], head_in_idx, flow_kind="core_to_head_in", magnitude=0.9)

    compression_idx = add_point(
        role="compression",
        x=MIDNIGHT_LANES["compression"],
        y=artifact_y,
        z=0.7,
        label="mixed-int + brotli artifact",
        stage="compression",
        kind="compression",
        layer_index=layers - 1,
        bitwidth=max(other_bits, embed_bits),
    )
    metric_idx = add_point(
        role="metric",
        x=MIDNIGHT_LANES["metric"],
        y=artifact_y + 0.55,
        z=-0.7,
        label="final sliding-window bpb",
        stage="metric",
        kind="metric",
        layer_index=layers - 1,
    )
    add_edge(head_in_idx, compression_idx, flow_kind="head_to_artifact", magnitude=0.85)
    add_edge(compression_idx, metric_idx, flow_kind="artifact_to_metric", magnitude=0.80)
    return points, edges
...
def build_points(
    ...
    *,
    ...
    if arch_spec and len(pts) < max_points:
        spec_points, spec_edges = _build_midnight_spec_points(arch_spec, step_norm=step_norm)
        remaining_points = max(0, max_points - len(pts))
        attach_points = spec_points[:remaining_points]
        base_idx = len(pts)
        pts.extend(attach_points)

        if attach_points and len(edges) < max_edges:
            remaining_edges = max(0, max_edges - len(edges))
            attach_count = len(attach_points)
            kept = 0
            for e in spec_edges:
                if kept >= remaining_edges:
                    break
                i = _safe_int(e.get("i"), -1)
                j = _safe_int(e.get("j"), -1)
                if i < 0 or j < 0 or i >= attach_count or j >= attach_count:
                    continue
                edge_copy = dict(e)
                edge_copy["i"] = base_idx + i
                edge_copy["j"] = base_idx + j
...
def main() -> None:
    ...
    ap.add_argument(
        "--spec-json",
        default="",
        help="Optional architecture spec JSON (e.g., midnight_12l_spec.json) to inject tagged architecture points.",
    )
    args = ap.parse_args()

    in_path = Path(args.input)
    out_path = Path(args.output)
    arch_spec = _load_optional_json(args.spec_json) if args.spec_json else None
    ...
    if arch_spec:
        payload["meta"]["architecture"] = arch_spec
