...
class Hyperparameters:
    ...
    export_reserve_ms = float(os.environ.get("EXPORT_RESERVE_MS", 30000.0))
    ...
    quant_roundtrip_eval = bool(int(os.environ.get("QUANT_ROUNDTRIP_EVAL", "1")))
...
def main() -> None:
    ...
    # Export path (quantization + optional GPTQ calibration + optional roundtrip eval)
    # must complete inside MAX_WALLCLOCK_SECONDS. We reserve time up front so training
    # behavior stays stable even when export settings change.
    _skip_gptq = int(os.environ.get("SKIP_GPTQ", "0"))
    _needs_lowbit_gptq = any(bits < 8 for bits in quant_policy.values())
    _export_reserve_ms = float(args.export_reserve_ms) if max_wallclock_ms is not None else 0.0
    effective_max_wallclock_ms = (max_wallclock_ms - _export_reserve_ms) if max_wallclock_ms is not None else None
    log0(
        f"export_profile:reserve_ms={_export_reserve_ms:.0f} "
        f"skip_gptq={int(_skip_gptq)} needs_lowbit_gptq={int(_needs_lowbit_gptq)} "
        f"quant_roundtrip_eval={int(args.quant_roundtrip_eval)} ngram_order={args.ngram_eval_order}"
    )
    ...
    # GPTQ calibration (optional): reads training data and runs inside EXPORT_RESERVE_MS.
    if _skip_gptq:
