...
#   2. Installs core Python deps plus zstandard/brotli
#   3. Installs or wires FlashAttention-3 if available
#   4. Downloads SP1024 and SP8192 data if the local data helper exists
...
for module_name in ("zstandard", "brotli", "sentencepiece"):
    try:
...
def check_tokenizer(filename: str, expected_vocab: int) -> None:
...
def check_dataset(variant: str) -> None:
...
check_tokenizer("fineweb_8192_bpe.model", 8192)
check_dataset("sp1024")
check_dataset("sp8192")
PYEOF
