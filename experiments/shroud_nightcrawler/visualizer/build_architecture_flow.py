...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--spec-json",
        default="",
        help="Optional architecture spec JSON to attach under meta.architecture.",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    if args.spec_json:
        ...
        if not spec_path.is_file():
            raise FileNotFoundError(f"missing spec JSON: {spec_path}")
        graph["meta"]["architecture"] = json.loads(spec_path.read_text(encoding="utf-8"))
