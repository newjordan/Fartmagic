...
def _serialize_quant_raw(quant_result: dict[str, Tensor], quant_meta: dict) -> bytes:
    """Serialize quantized model to raw bytes — bypasses torch.save ZIP overhead."""
    import json, struct as _struct
    tensor_dir = {}
    chunks = []
    offset = 0
...
def _deserialize_quant_raw(data: bytes) -> tuple[dict[str, Tensor], dict]:
    ...
    import json, struct as _struct
    _np_dtypes = {"torch.int8": np.int8, "torch.float16": np.float16, "torch.float32": np.float32,
