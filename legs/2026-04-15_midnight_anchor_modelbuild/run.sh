...
reject_adhoc_env() {
  local vars=(
    MAX_WALLCLOCK_SECONDS SKIP_GPTQ POST_EMA_DIAGNOSTIC SKIP_FINAL_EVAL
...
if [[ -n "${LEG_DIFF_GUARD_ARGS:-}" ]]; then
  ...
  python3 "${REPO_ROOT}/scripts/leg_diff_guard.py" "${LEG_DIR}" --max-code-changes 120 --max-total-changed-lines 180
fi
