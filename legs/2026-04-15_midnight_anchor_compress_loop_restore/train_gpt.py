...
def main() -> None:
    ...
    if not args.init_model_path:
        raise RuntimeError("midnight_anchor_compress requires INIT_MODEL_PATH to point at a finished modelbuild checkpoint")
    if args.init_model_path:
        ...
        if args.num_loops > 0 and hasattr(base_model, "looping_active"):
            base_model.looping_active = True
            log0(f"init_model:loop_state_restored looping:{int(bool(base_model.looping_active))}")
    ...
    if args.iterations != 0:
        raise RuntimeError("midnight_anchor_compress is export-only; set ITERATIONS=0 in tracked_env.sh")
    if args.warmup_steps != 0:
        raise RuntimeError("midnight_anchor_compress is export-only; set WARMUP_STEPS=0 in tracked_env.sh")
    if args.ngram_eval_order != 0:
        raise RuntimeError("midnight_anchor_compress disables legacy ngram eval; set NGRAM_EVAL_ORDER=0")
    ...
    if hasattr(base_model, "looping_active") and hasattr(eval_model, "looping_active"):
        eval_model.looping_active = base_model.looping_active
        log0(f"eval_loop_state:restored looping:{int(bool(eval_model.looping_active))}")
    else:
        log0("eval_loop_state:not_applicable")
    torch.cuda.synchronize()
    ...
    if args.skip_final_eval:
        log0("final_eval:skipped deployed sliding eval by SKIP_FINAL_EVAL=1")
    else:
        log0("final_eval:deployed_artifact_only ngram_eval=disabled")
