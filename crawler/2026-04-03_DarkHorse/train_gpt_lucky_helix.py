...
class Hyperparameters:
    ...
    # Helix: cheap MLP crawler co-firing with cross-injection
    helix_enabled = bool(int(os.environ.get("HELIX", "0")))
    helix_dim = int(os.environ.get("HELIX_DIM", 64))
    helix_stride = int(os.environ.get("HELIX_STRIDE", 2))
...
class HelixCrawler(nn.Module):
    """Cheap MLP refinement module for Helix cross-injection.
    NOT a full transformer — no attention. Just: normalize → project down → activate → project up.
    The crawler's job is to process hints from the flat stream and produce refinements back.
    It's a learned residual correction pathway, not a second model."""
    def __init__(self, model_dim: int, helix_dim: int):
        super().__init__()
        self.norm = RMSNorm()
        self.down = nn.Linear(model_dim, helix_dim * 2, bias=False)  # 2× for gated
        self.up = nn.Linear(helix_dim, model_dim, bias=False)
        nn.init.zeros_(self.up.weight)  # warm start: crawler does nothing initially
    def forward(self, x: Tensor) -> Tensor:
        h = self.norm(x)
        h = self.down(h)
        gate, val = h.chunk(2, dim=-1)
        h = F.silu(gate) * val  # SwiGLU-style gating
        return x + self.up(h)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Helix: cheap crawler + cross-injection bridge
        self.helix_enabled = bool(int(os.environ.get("HELIX", "0")))
        self.helix_stride = int(os.environ.get("HELIX_STRIDE", 2))
        if self.helix_enabled:
            hdim = int(os.environ.get("HELIX_DIM", 64))
            self.crawler = HelixCrawler(model_dim, hdim)
            # Bridge: position-agnostic linear projections
            self.helix_f2c_down = nn.Linear(model_dim, hdim, bias=False)
            self.helix_f2c_up = nn.Linear(hdim, model_dim, bias=False)
            self.helix_c2f_down = nn.Linear(model_dim, hdim, bias=False)
            self.helix_c2f_up = nn.Linear(hdim, model_dim, bias=False)
            self.helix_merge = nn.Parameter(torch.zeros(model_dim))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            step += 1
            if self.helix_enabled and step % self.helix_stride == 0:
                x_crawl = x_crawl + self.helix_f2c_up(self.helix_f2c_down(x))
                x_crawl = self.crawler(x_crawl)
                x = x + self.helix_c2f_up(self.helix_c2f_down(x_crawl))
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            if self.helix_enabled and step % self.helix_stride == 0:
                ...
                x = x + self.helix_c2f_up(self.helix_c2f_down(x_crawl))
        if self.helix_enabled:
            m = torch.sigmoid(self.helix_merge).to(dtype=x.dtype)[None, None, :]
            x = x + m * x_crawl
        x = self.final_norm(x)
