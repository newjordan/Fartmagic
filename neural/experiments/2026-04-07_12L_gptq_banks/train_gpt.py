...
def gptq_calibrate(model: nn.Module, train_pattern: str, device: torch.device,
    ...
    """Collect Hessian H = X^T X for linear layers AND bank slices using training data.

    Banks (qo_bank, kv_bank, mlp_up_bank, mlp_down_bank) are nn.Parameter tensors
    used via F.linear, not nn.Module instances. We instrument F.linear during
    calibration to capture inputs for each bank slice, keyed as 'bank_name.slice_idx'.
    """
    hessians: dict[str, Tensor] = {}
    n_seen: dict[str, int] = {}
    hooks = []
    # --- Hook regular Linear/CastedLinear modules ---
    ...
    # --- Build data_ptr → bank key map for F.linear interception ---
    ...
    def _instrumented_linear(input, weight, bias=None):
        ptr = weight.data_ptr()
        key = bank_ptr_map.get(ptr)
        if key is not None:
            x = input.detach().float()
            if x.ndim == 3:
                x = x.reshape(-1, x.shape[-1])
            if key not in hessians:
                hessians[key] = torch.zeros(x.shape[1], x.shape[1], device=x.device, dtype=torch.float32)
                n_seen[key] = 0
            hessians[key].addmm_(x.t(), x)
            n_seen[key] += x.shape[0]
    ...
    log0 = lambda msg: print(msg, flush=True) if int(os.environ.get("RANK", "0")) == 0 else None
    n_bank = sum(1 for k in hessians if "." in k and any(b in k for b in ("qo_bank", "kv_bank", "mlp_up_bank", "mlp_down_bank")))
    n_module = len(hessians) - n_bank
    log0(f"gptq:hessians bank_slices={n_bank} modules={n_module}")
    model.train()
    return hessians
...
def mixed_quantize_gptq(state_dict: dict[str, Tensor], bit_policy: dict[str, int],
    ...
    for name, tensor in state_dict.items():
        ...
        if bits == 8:
            ...
            continue
        clip_range = quant_clip_range(bits)
        if t.ndim == 3 and any(f"{name}.{si}" in hessians for si in range(t.shape[0])):
            # Bank tensor: per-slice GPTQ quantization
            all_q, all_s = [], []
            for si in range(t.shape[0]):
                slice_2d = t[si]
                H = hessians.get(f"{name}.{si}")
                if H is not None and H.shape[0] == slice_2d.shape[1]:
                    q, s = gptq_quantize_weight(slice_2d, H.cpu(), clip_range=clip_range)
                    gptq_count += 1
                else:
                    q, s = quantize_lowbit_per_row(slice_2d, bits)
                    naive_count += 1
                all_q.append(q)
                all_s.append(s)
            result[name + ".q"] = torch.cat(all_q, dim=0)
            result[name + ".scale"] = torch.cat(all_s, dim=0)
