...
# Concept Arm A — Long Context Training (4096 vs baseline 2048)
export SEED="${SEED:-300}"
export NPROC_PER_NODE="${NPROC_PER_NODE:-4}"
export ITERATIONS=2000
export MAX_WALLCLOCK_SECONDS=9999
export RUN_ID="concept_arm_a_longctx_seed${SEED}"

pip install brotli -q 2>/dev/null || true

TRAIN_SCRIPT="$(find "${REPO_ROOT}" -path "*/arm_a_longctx/train_gpt.py" | head -1)"
