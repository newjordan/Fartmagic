#!/bin/bash
set -euo pipefail
# FA_WING_GREEN_DN_1: Flow Instructions + DeltaNet on A-Wing SOTA base
# Frugendorff crawler (4 flat + 1 shared x 4 loops) with flow instructions + DeltaNet memory:
#   inst_k = up_k(proj(x))  — recomputed from CURRENT x each loop
#   delta_net: S carried between loops, error-corrects activations each pass
# H0 confirmed (flow inst): quant gap +0.006 BPB (vs +2.93 static instructions)
...
echo "============================================"
echo "  FA_WING_GREEN_DN_1 — Flow Instructions + DeltaNet + A-Wing SOTA base"
echo "  Seed: ${SEED}"
echo "  inst_dim=32 FLOW + delta_net_heads=4 | 4 flat + 1 crawler x 4 loops"
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
    ...
    2>&1 | tee "logs/fawing_greendn1_s${SEED}_$(date +%Y%m%d_%H%M%S).log"
