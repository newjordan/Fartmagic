        ...
        if distributed:
            for name in sorted(self.hessian_sums):
                ...
                count_tensor = torch.tensor(
                    float(self.n_seen.get(name, 0)),
                    device=device,
                    dtype=torch.float32,
                )
                dist.all_reduce(count_tensor, op=dist.ReduceOp.SUM)
                self.hessian_sums[name] = H
                self.n_seen[name] = int(count_tensor.item())
        out: dict[str, Tensor] = {}
        for name, H in self.hessian_sums.items():
            seen = self.n_seen.get(name, 0)
            if seen <= 0:
                continue
            out[name] = (H / float(seen)).to(dtype=torch.float32, device="cpu")
        return out
