...
LEG_DIR="${REPO_ROOT}/legs/2026-04-15_midnight_anchor_modelbuild"
...
reject_adhoc_env() {
  local vars=(
    ITERATIONS WARMDOWN_ITERS MAX_WALLCLOCK_SECONDS SKIP_GPTQ POST_EMA_DIAGNOSTIC SKIP_FINAL_EVAL
...
if [[ -n "${LEG_DIFF_GUARD_ARGS:-}" ]]; then
  ...
  python3 "${REPO_ROOT}/scripts/leg_diff_guard.py" "${LEG_DIR}" --max-code-changes 10 --max-total-changed-lines 30
fi
