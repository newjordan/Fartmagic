...
class Hyperparameters:
    ...
    recur_layers = os.environ.get("RECUR_LAYERS", "4,5")  # depth recurrence: re-fire these physical layers
...
class GPT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self._recur_layers = [int(x) for x in recur_layers.split(",") if x.strip()] if recur_layers else []
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        # Depth recurrence: re-fire specified layers (shared weights, same blocks)
        for phys_i in self._recur_layers:
            ve = self._get_ve(phys_i, input_ids, ve_cache)
            x, _ = self.blocks[phys_i](x, x0,
                self.qo_bank[phys_i], self.kv_bank[phys_i], self.kv_bank[n + phys_i],
                self.qo_bank[n + phys_i], self.mlp_up_bank[phys_i], self.mlp_down_bank[phys_i],
                v_embed=ve, v0=v0)
