...
class CastedLinear(nn.Linear):
    ...
    _noisy_qat: bool = bool(int(os.environ.get("NOISY_QAT", "0")))
    _is_crawler_block: bool = False  # set by CrawlerGPT.__init__ for crawler blocks
    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            if CastedLinear._noisy_qat and self._is_crawler_block:
                # Noisy QAT (Evangeline Kamin, PR #363): differentiable noise calibrated
                # to int6 quantization step size. Applied to crawler blocks only.
                # Collapses quant gap from 0.37 to 0.002 BPB on looped architectures.
                with torch.no_grad():
                    amax = self.weight.float().abs().amax(dim=1, keepdim=True).clamp_min(1e-12)
                    step_size = amax / 31.0  # int6 range [-31, 31]
                noise = (torch.rand_like(w) - 0.5) * step_size.to(w.dtype)
                w = w + noise
            else:
                # Standard STE QAT for flat blocks
                with torch.no_grad():
...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        # Mark all CastedLinear modules inside crawler blocks for Noisy QAT
        for block in self.crawler_blocks:
            for module in block.modules():
                if isinstance(module, CastedLinear):
                    module._is_crawler_block = True
        self.final_norm = RMSNorm()
        self.lm_head = None if tie_embeddings else CastedLinear(model_dim, vocab_size, bias=False)
        if self.lm_head is not None:
            self.lm_head._zero_init = True
    ...
    def _run_crawler(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict,
        ...
        for loop in range(self.crawler_loops):
            ...
            x_prev_loop = x_loop
            # SCORE contractive recurrence (arXiv 2603.10544):
            # x' = (1-dt)*x + dt*F(x) instead of x' = F(x)
            dt = float(os.environ.get("CRAWLER_CONTRACTIVE_DT", "1.0"))
            if dt < 1.0:
                x = (1.0 - dt) * x + dt * x_loop
            else:
                x = x_loop
        return x
...
def quant_clip_range(bits: int) -> int:
    ...
    return (1 << (bits - 1)) - 1
def quantize_lowbit_per_row(t: Tensor, bits: int) -> tuple[Tensor, Tensor]:
...
def mixed_quantize_gptq(state_dict: dict[str, Tensor], bit_policy: dict[str, int],
                        hessians: dict[str, Tensor],
                        crawler_int8: bool = False) -> tuple[dict, dict]:
    """Mixed-bit export with per-category bit widths and GPTQ when Hessians available."""
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    gptq_count, naive_count = 0, 0
    ...
    for name, tensor in state_dict.items():
        ...
        if crawler_int8 and name.startswith("crawler_blocks.") and t.is_floating_point() and t.numel() > 65536:
            ...
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8", "bits": 8}
            quant_counts[8] = quant_counts.get(8, 0) + 1
            continue
        bits = normalize_quant_bits(bit_policy.get(cat, bit_policy.get("other", 8)))
        if bits >= 16:
        ...
        if t.ndim == 2:
            ...
            if H is not None and H.shape[0] == t.shape[1]:
                ...
                q, s = quantize_lowbit_per_row(t, bits)
                naive_count += 1
            result[name + ".q"] = q
            result[name + ".scale"] = s
            ...
            naive_count += 1
            quant_counts[bits] = quant_counts.get(bits, 0) + 1
    quant_summary = " ".join(f"int{bits}:{count}" for bits, count in sorted(quant_counts.items()))
    print(f"mixed_quantize: {gptq_count} GPTQ, {naive_count} naive {quant_summary}".strip(), flush=True)
    return result, meta
...
def main() -> None:
    ...
    # Mixed-bit quant policy: per-category bit widths (default all int6 = BWX behavior)
    ...
    quant_embed_bits = int(os.environ.get("QUANT_EMBED_BITS", "6"))
    quant_other_bits = int(os.environ.get("QUANT_OTHER_BITS", "8"))
    use_mixed_bit = any(b != 6 for b in [quant_attn_bits, quant_mlp_bits, quant_aux_bits, quant_embed_bits])
    if args.skip_gptq:
        quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn", "aux"})
    elif use_mixed_bit and not args.skip_gptq:
        bit_policy = {
            "attn": normalize_quant_bits(quant_attn_bits),
            "mlp": normalize_quant_bits(quant_mlp_bits),
            "aux": normalize_quant_bits(quant_aux_bits),
            "embed": normalize_quant_bits(quant_embed_bits),
            "other": normalize_quant_bits(quant_other_bits),
        }
        log0(f"quant_policy: {' '.join(f'{k}=int{v}' for k,v in bit_policy.items())}")
        quant_result, quant_meta = mixed_quantize_gptq(
            sd_cpu, bit_policy, gptq_hessians,
