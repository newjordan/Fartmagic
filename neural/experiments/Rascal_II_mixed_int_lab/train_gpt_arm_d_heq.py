...
def _find_best_row_scales(W: Tensor, clip_range: int = 31) -> Tensor:
    """HEQ-inspired scale selection (arXiv 2501.04517): maximize entropy of quantized
    representation by searching over many more candidate scales, including n-quantile
    boundaries that equalize bin occupancy. Falls back to MSE tiebreak."""
    t32 = W.float()
    n_levels = 2 * clip_range + 1
    best_s = t32.abs().amax(dim=1) / clip_range
    best_s = best_s.clamp_min(1.0 / clip_range)
    best_score = torch.full((t32.shape[0],), float('-inf'))
    # Candidate percentiles: original 5 + HEQ n-quantile boundaries
    pcts = [0.90, 0.95, 0.98, 0.99, 0.995, 0.9975, 0.999, 0.9995, 0.9999, 0.99999, 1.0]
    for pct in pcts:
        if pct < 1.0:
        ...
        # Entropy: count how evenly bins are used (higher = better representation)
        q_shifted = (q + clip_range).long()  # shift to [0, n_levels-1]
        entropy = torch.zeros(t32.shape[0], device=t32.device)
        for row_idx in range(t32.shape[0]):
            counts = torch.bincount(q_shifted[row_idx], minlength=n_levels).float()
            probs = counts / counts.sum()
            probs = probs[probs > 0]
            entropy[row_idx] = -(probs * probs.log()).sum()
        # Score: entropy - lambda * mse (entropy-first, mse tiebreak)
        max_entropy = math.log(n_levels)
        norm_entropy = entropy / max_entropy
        mse_norm = mse / (t32.pow(2).mean(dim=1).clamp_min(1e-10))
        score = norm_entropy - 0.1 * mse_norm
