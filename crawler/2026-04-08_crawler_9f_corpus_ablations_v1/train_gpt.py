...
class Hyperparameters:
    ...
    # BW23: QAT surrogate ablation knobs
    qat_surrogate = os.environ.get("QAT_SURROGATE", "legacy").strip().lower()
    qat_sigmoid_beta = float(os.environ.get("QAT_SIGMOID_BETA", "6.0"))
    ...
    # BW23: sensitivity-style mixed-bit policy
    int6_cats = os.environ.get("INT6_CATS", "mlp,attn,aux")
...
class CastedLinear(nn.Linear):
    ...
    _qat_surrogate: str = "legacy"
    _qat_sigmoid_beta: float = 6.0

    @staticmethod
    def _soft_round_sigmoid(x: Tensor, beta: float) -> Tensor:
        x_floor = torch.floor(x)
        return x_floor + torch.sigmoid(beta * (x - x_floor - 0.5))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            ...
            with torch.no_grad():
                # Keep clipping policy aligned with export quantizer calibration.
                row_clip = torch.quantile(w32.abs(), 0.9995, dim=1)
                scale = (row_clip / 31.0).clamp_min(1.0 / 31.0)
            scaled = w32 / scale[:, None]
            surrogate = CastedLinear._qat_surrogate
            if surrogate == "legacy":
                with torch.no_grad():
                    q_hard = torch.clamp(torch.round(scaled), -32, 31)
                    w_q = (q_hard * scale[:, None]).to(x.dtype)
                w = w + (w_q - w).detach()
            elif surrogate == "softclamp":
                q_soft = 31.0 * torch.tanh(scaled / 31.0)
                w = (q_soft * scale[:, None]).to(x.dtype)
            elif surrogate in ("sigmoidste", "sigmoid_ste"):
                q_soft = CastedLinear._soft_round_sigmoid(
                    scaled, CastedLinear._qat_sigmoid_beta
                )
                q_soft = torch.clamp(q_soft, -31.0, 31.0)
                w = (q_soft * scale[:, None]).to(x.dtype)
            else:
                raise ValueError(f"Unknown QAT_SURROGATE='{surrogate}'")
...
VALID_INT6_CATS = {"embed", "aux", "mlp", "attn", "other"}


def parse_int6_cats(raw: str) -> set[str]:
    default = {"mlp", "attn", "aux"}
    if raw is None:
        return default
    tokens = []
    ...
    if not tokens:
        return default
    if "all" in tokens:
        return set(VALID_INT6_CATS)
    cats = set(tokens)
    unknown = cats - VALID_INT6_CATS
    if unknown:
        allowed = ",".join(sorted(VALID_INT6_CATS))
        bad = ",".join(sorted(unknown))
        raise ValueError(f"Unknown INT6_CATS token(s): {bad}. Allowed: {allowed}")
    return cats
...
def main() -> None:
    ...
    CastedLinear._qat_surrogate = args.qat_surrogate
    CastedLinear._qat_sigmoid_beta = args.qat_sigmoid_beta
    int6_cats = parse_int6_cats(args.int6_cats)
    ...
    log0(
        f"qat:enabled={int(args.qat_enabled)} surrogate={args.qat_surrogate} "
        f"sigmoid_beta={args.qat_sigmoid_beta}"
    )
    log0(f"quant:int6_cats={','.join(sorted(int6_cats))}")
    ...
    if args.skip_gptq:
        ...
        quant_result, quant_meta = mixed_quantize_int6_gptq(
            sd_cpu, int6_cats, gptq_hessians,
