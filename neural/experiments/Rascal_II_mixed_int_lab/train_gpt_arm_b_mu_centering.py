...
def main() -> None:
    ...
    while True:
        ...
        optimizer_muon.step()
        zero_grad_all()
        # mu-centering: subtract mean output embedding after optimizer step (arXiv 2601.02031)
        # With tied embeddings, tok_emb.weight IS the output projection.
        # Zero hyperparameters, ~0.3% overhead. Enables higher effective LR.
        with torch.no_grad():
            base_model.tok_emb.weight.sub_(base_model.tok_emb.weight.mean(dim=0, keepdim=True))
        # EMA update
        with torch.no_grad():
