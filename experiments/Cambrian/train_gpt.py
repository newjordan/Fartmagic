...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if getattr(self, 'use_delta', False):
            # GatedDeltaNet: ignores bank weights, has its own projections
...
class GatedDeltaNet(nn.Module):
    ...
    Processes sequence in chunks; state passes between chunks at seams.
    S_t = S_{t-1} + v_t ⊗ k_t - β_t*(S_{t-1}k_t) ⊗ k_t
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        C = self.chunk_size
        num_chunks = T // C  # static — T=2048, C=64 → 32 chunks

        q = self.q_proj(x).reshape(B, T, H, d).permute(0, 2, 1, 3)  # (B,H,T,d)
        k = F.normalize(self.k_proj(x).reshape(B, T, H, d).permute(0, 2, 1, 3), dim=-1)
        ...
        for ci in range(num_chunks):
            ...
            for t in range(C):
                ...
                # Read current stored value at k_t
                r_t = torch.einsum('bhij,bhj->bhi', S, kt)  # (B, H, d)
                # Erase old, write new
                S = S - bt.unsqueeze(-1).unsqueeze(-1) * torch.einsum('bhi,bhj->bhij', r_t, kt)
                S = S + torch.einsum('bhi,bhj->bhij', vt, kt)
                # Read output using query
                y_t = torch.einsum('bhij,bhj->bhi', S.to(qci.dtype), qci[:, :, t, :])
        ...
        out = self.norm(out).reshape(B, T, D)
...
class GPT(nn.Module):
    def __init__(
        ...
        # --- Cambrian: install GatedDeltaNet in bottom N layers ---
        ...
        if xsa_last_n > 0:
            for i in range(max(0, num_layers - xsa_last_n), num_layers):
                # XSA only applies to standard attention blocks (not GatedDeltaNet)
                if not getattr(self.blocks[i], 'use_delta', False):
...
def main() -> None:
    ...
    # Collect GDN matrix params explicitly (they are 2D but not in banks)
    _gdn_param_ids: set[int] = set()
    _gdn_matrix_params: list = []
    for block in base_model.blocks:
        if getattr(block, 'use_delta', False):
            for p in block.attn.parameters():
                if id(p) not in _gdn_param_ids:
                    _gdn_param_ids.add(id(p))
                    if p.ndim >= 2:
                        _gdn_matrix_params.append(p)
    scalar_params = [
        p
