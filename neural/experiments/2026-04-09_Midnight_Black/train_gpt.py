...
_QUANT_OUTLIER_PRESERVE = bool(int(os.environ.get("QUANT_OUTLIER_PRESERVE", "1")))
def _find_best_row_scales(W: Tensor, clip_range: int = 31) -> Tensor:
    t32 = W.float()
    if _QUANT_OUTLIER_PRESERVE:
        best_s = t32.abs().amax(dim=1) / clip_range
        return best_s.clamp_min(1.0 / clip_range)
    best_s = t32.abs().amax(dim=1) / clip_range
...
def gptq_calibrate(model: nn.Module, train_pattern: str, device: torch.device,
    ...
    """Collect Hessian H = X^T X for linear layers AND bank parameter projections."""
    hessians: dict[str, Tensor] = {}
    n_seen: dict[str, int] = {}
    hooks = []
    def _accumulate(key: str, x: Tensor):
        x = x.detach().float()
        if x.ndim == 3:
    ...
    def make_hook(name: str):
        def hook_fn(module, inp, out):
            _accumulate(name, inp[0])
        return hook_fn
    for name, module in model.named_modules():
        if isinstance(module, (nn.Linear, CastedLinear)):
            hooks.append(module.register_forward_hook(make_hook(name)))
    # Hook block.attn and block.mlp for bank parameter calibration
    for layer_idx, block in enumerate(model.blocks):
        def _make_attn_hook(idx):
            def hook_fn(module, inp, out):
                _accumulate(f"bank_attn_{idx}", inp[0])
            return hook_fn
        def _make_mlp_hook(idx):
            def hook_fn(module, inp, out):
                _accumulate(f"bank_mlp_{idx}", inp[0])
            return hook_fn
...
def mixed_quantize_gptq(state_dict: dict[str, Tensor], bit_policy: dict[str, int],
    ...
    for name, tensor in state_dict.items():
        ...
        if bits == 8:
            ...
            continue
        clip_range = quant_clip_range(bits)
        # Per-slice GPTQ for 3D bank parameters
        if t.ndim == 3 and any(bk in name for bk in ("qo_bank", "kv_bank", "mlp_up_bank")):
            num_slices = t.shape[0]
            n_layers = num_slices // 2 if ("qo_bank" in name or "kv_bank" in name) else num_slices
            all_q, all_s = [], []
            for si in range(num_slices):
                slice_2d = t[si]
                H_key = None
                if "qo_bank" in name or "kv_bank" in name:
                    layer = si if si < n_layers else si - n_layers
                    H_key = f"bank_attn_{layer}"
                elif "mlp_up_bank" in name:
                    H_key = f"bank_mlp_{si}"
                H = hessians.get(H_key) if H_key else None
                if H is not None and H.shape[0] == slice_2d.shape[1]:
                    sq, ss = gptq_quantize_weight(slice_2d, H.cpu(), clip_range=clip_range)
                    gptq_count += 1
                else:
                    sq, ss = quantize_lowbit_per_row(slice_2d, bits)
                    naive_count += 1
                all_q.append(sq)
                all_s.append(ss)
