...
class Hyperparameters:
    ...
    late_quant_mlp_bits = int(os.environ.get("LATE_QUANT_MLP_BITS", 8))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mlp_up_bank = nn.Parameter(torch.empty(n_total, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(n_total, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        n = self.total_layers  # init ALL layers including late
        proj_scale = 1.0 / math.sqrt(2 * self.num_layers)  # scale based on core layers
        ...
        if self.late_layers_active and self.late_layers > 0:
            for i in range(self.num_layers, self.total_layers):
                x, _ = self.blocks[i](x, x0,
                    ...
                    self.qo_bank[nt + i], self.mlp_up_bank[i], self.mlp_down_bank[i])
        x = self.final_norm(x)
...
def main() -> None:
    ...
    while True:
        ...
        if args.late_layers > 0 and not base_model.late_layers_active:
            frac_late = step / max(args.iterations, 1) if max_wallclock_ms is None else elapsed_ms / max(max_wallclock_ms, 1e-9)
            if frac_late >= args.late_activate_at:
                base_model.late_layers_active = True
                log0(f"late_layers:enabled step:{step} frac:{frac_late:.3f} layers:{args.num_layers}-{args.num_layers + args.late_layers - 1}")
