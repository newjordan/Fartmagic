...
# Hybrid attention: whale Triton fwd + FA3 CUDA bwd. Wins fwd by 13-15% at
# T<=2048 (B=2 H=8 KV=4 D=64 bf16 GQA causal) on H100 SXM, Triton 3.6.
# Bench evidence: legs/2026-04-16_whale_pod_autoresearch/evidence/hybrid_sweep.json
try:
    from vault.whale_kernel_triton import whale_fwd_fa3_bwd
except ImportError:
    whale_fwd_fa3_bwd = None
try:
    import zstandard
except ImportError:
    zstandard = None
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if whale_fwd_fa3_bwd is not None:
            # Hybrid path: whale Triton fwd + FA3 CUDA bwd. Same I/O layout
            # as flash_attn_3_func: (B, T, H, D) bf16/fp16, returns (B, T, H, D).
            ...
            y = whale_fwd_fa3_bwd(q_attn, k_attn, v_attn, causal=True)
        elif flash_attn_3_func is not None:
            q_attn, k_attn, v_attn = q, k, v
            if q_attn.dtype not in (torch.float16, torch.bfloat16):
                q_attn = q_attn.to(torch.bfloat16)
                k_attn = k_attn.to(torch.bfloat16)
