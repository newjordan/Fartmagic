            ...
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    if _use_learned_alpha:
                        ...
                        # Original backoff: highest matching order wins
...
def main() -> None:
    ...
    train_mixer: TrainNgramOracle | None = None
    if args.mixer_enabled:
        ...
        prefill_mode = "rank0+broadcast" if distributed else "single-rank"
        log0(
            "mixer:prefill "
        ...
        if (not distributed) or rank == 0:
            ...
            for fi, f in enumerate(train_files):
                ...
                if rank == 0 and ((fi + 1) % 5 == 0 or fi == 0 or fi + 1 == len(train_files)):
                    elapsed = time.perf_counter() - t_prefill
                    toks_per_s = train_mixer.total_tokens / max(elapsed, 1e-9)
                    print(
                        f"  mixer:prefill {fi+1}/{len(train_files)} shards, "
                ...
                if prefill_cap_s > 0.0 and local_prefilled_shards >= prefill_min_shards:
                    ...
                    if elapsed >= prefill_cap_s:
                        if rank == 0:
                            print(
                                f"  mixer:prefill cutoff at {local_prefilled_shards} shards "
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            # Mixer: get n-gram probs from training oracle (CPU, outside compiled model)
            _mx_p, _mx_v = None, None
            if train_mixer is not None:
                _mx_p_cpu, _mx_v_cpu = train_mixer.get_ngram_probs(x, y)
                _mx_p = _mx_p_cpu.to(device=device, dtype=torch.bfloat16)
                _mx_v = _mx_v_cpu.to(device=device)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
