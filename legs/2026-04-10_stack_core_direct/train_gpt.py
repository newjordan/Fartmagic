...
class Hyperparameters:
    ...
    max_wallclock_seconds = float(os.environ.get("MAX_WALLCLOCK_SECONDS", 360.0))
    ...
    skip_final_eval = bool(int(os.environ.get("SKIP_FINAL_EVAL", "1")))
    post_ema_diagnostic = bool(int(os.environ.get("POST_EMA_DIAGNOSTIC", "0")))
    skip_quant_roundtrip_eval = bool(int(os.environ.get("SKIP_QUANT_ROUNDTRIP_EVAL", "1")))
...
def main() -> None:
    ...
    if distributed:
        dist.barrier()
    if args.skip_quant_roundtrip_eval:
        log0("quant_roundtrip_eval:skipped SKIP_QUANT_ROUNDTRIP_EVAL=1")
        del sd_cpu
    else:
