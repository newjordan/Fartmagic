...
def _qat_fake_int6(w: Tensor) -> Tensor:
    """STE fake-quantize a 2D weight to int6 range. Percentile clipping matches actual quant."""
    with torch.no_grad():
        w32 = w.float()
...
def _qat_bank(bank: Tensor) -> Tensor:
    """Apply int6 STE fake-quantize to a 3D parameter bank (reshape to 2D, quantize, reshape back)."""
    orig_shape = bank.shape
    w = bank.reshape(-1, bank.shape[-1])
    result = _qat_fake_int6(w)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        # Bank QAT: fake-quantize all weight banks during late training
        qo = _qat_bank(self.qo_bank) if CastedLinear._qat_enabled and self.training else self.qo_bank
        kv = _qat_bank(self.kv_bank) if CastedLinear._qat_enabled and self.training else self.kv_bank
        mlp_up = _qat_bank(self.mlp_up_bank) if CastedLinear._qat_enabled and self.training else self.mlp_up_bank
        mlp_down = _qat_bank(self.mlp_down_bank) if CastedLinear._qat_enabled and self.training else self.mlp_down_bank
...
def eval_val_sliding(
    ...
    If slot_enabled, applies Context-Only SLOT at each window (except window 0):
    optimizes an additive hidden-state delta on already-scored context positions only,
    then scores new positions under that delta. Causally safe. Model weights unchanged.
    max_windows > 0 limits evaluation to the first N windows per rank (ablations only).
    """
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
        ...
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            if slot_enabled and not any(ws == 0 for ws in batch_ws):
                ...
                _hook = base_model.final_norm.register_forward_hook(
                    lambda m, i, o: _cap.__setitem__(0, o.detach())
                )
                with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    base_model.forward_logits(x_batch)
                _hook.remove()
                hidden = _cap[0]  # (bsz, seq_len, dim), bf16
                ...
                with torch.enable_grad():
                    for _ in range(slot_steps):
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            ...
                            logits_s = base_model.logit_softcap * torch.tanh(
                                lp / base_model.logit_softcap)
                        F.cross_entropy(logits_s[ctx_mask].float(),
                                        y_batch[ctx_mask]).backward()
                        opt.step()
                with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
