...
class Hyperparameters:
    ...
    quant_attn_bits = int(os.environ.get("QUANT_ATTN_BITS", 5))   # Rascal III: int5 attn
    quant_mlp_bits = int(os.environ.get("QUANT_MLP_BITS", 6))
    quant_aux_bits = int(os.environ.get("QUANT_AUX_BITS", 6))
    quant_embed_bits = int(os.environ.get("QUANT_EMBED_BITS", 8))  # Rascal III: int8 embed
    ...
    transition_integrator = os.environ.get("TRANSITION_INTEGRATOR", "euler").strip().lower()
    transition_hybrid_last_n = int(os.environ.get("TRANSITION_HYBRID_LAST_N", 4))
    longctx_taps_enabled = bool(int(os.environ.get("LONGCTX_TAPS_ENABLED", "0")))
    longctx_tap_offsets = os.environ.get("LONGCTX_TAP_OFFSETS", "64,256,1024")
    longctx_tap_prefix_mean = bool(int(os.environ.get("LONGCTX_TAP_PREFIX_MEAN", "1")))
    longctx_tap_norm = bool(int(os.environ.get("LONGCTX_TAP_NORM", "1")))
...
class Block(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        integrator_mode: str = "euler",
        longctx_taps_enabled: bool = False,
        longctx_tap_offsets: str = "64,256,1024",
        longctx_tap_prefix_mean: bool = True,
        longctx_tap_norm: bool = True,
        ...
        self.longctx_taps_enabled = longctx_taps_enabled
        self.longctx_tap_norm = longctx_tap_norm
        offsets: list[int] = []
        if longctx_taps_enabled:
            for raw in longctx_tap_offsets.split(","):
                s = raw.strip()
                if not s:
                    continue
                try:
                ...
                if v > 0:
                    offsets.append(v)
        self.longctx_tap_offsets = tuple(sorted(set(offsets)))
        enable_any_tap = bool(self.longctx_tap_offsets) or bool(longctx_tap_prefix_mean)
        if longctx_taps_enabled and self.longctx_tap_offsets:
            self.longctx_tap_logits = nn.Parameter(
                torch.full((len(self.longctx_tap_offsets),), longctx_tap_logit_init, dtype=torch.float32)
            )
        else:
            self.register_parameter("longctx_tap_logits", None)
        if longctx_taps_enabled and longctx_tap_prefix_mean:
            self.longctx_prefix_logit = nn.Parameter(torch.tensor(longctx_prefix_logit_init, dtype=torch.float32))
        else:
            self.register_parameter("longctx_prefix_logit", None)
        if longctx_taps_enabled and enable_any_tap:
    ...
    def _apply_long_context(self, x_state: Tensor) -> Tensor:
        if not self.longctx_taps_enabled or self.longctx_tap_scale is None:
            return x_state
        bsz, seqlen, _ = x_state.shape
        ctx: Tensor | None = None
        if self.longctx_tap_logits is not None:
            for i, off in enumerate(self.longctx_tap_offsets):
                if off >= seqlen:
                    continue
                shifted = torch.cat([torch.zeros_like(x_state[:, :off]), x_state[:, :-off]], dim=1)
                gate = torch.sigmoid(self.longctx_tap_logits[i]).to(dtype=x_state.dtype)
                term = gate * shifted
                ctx = term if ctx is None else (ctx + term)
        if self.longctx_prefix_logit is not None:
            prefix = torch.cumsum(x_state, dim=1)
            denom = torch.arange(1, seqlen + 1, device=x_state.device, dtype=x_state.dtype)[None, :, None]
            prefix = prefix / denom
            gate = torch.sigmoid(self.longctx_prefix_logit).to(dtype=x_state.dtype)
            term = gate * prefix
            ctx = term if ctx is None else (ctx + term)
        if ctx is None:
            return x_state
        if self.longctx_tap_norm:
            ctx = F.rms_norm(ctx, (ctx.size(-1),))
        return x_state + self.longctx_tap_scale.to(dtype=x_state.dtype)[None, None, :] * ctx
    def _core_transition(
        ...
        x_state = self._apply_long_context(x_state)
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        mode = self.integrator_mode

        if mode == "euler":
            return self._core_transition(x_in, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=v_embed, v0=v0)

        x1, raw_v = self._core_transition(x_in, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=v_embed, v0=v0)
        k1 = x1 - x_in

        if mode == "rk2":
            x2_in = x_in + 0.5 * k1
            x2, _ = self._core_transition(x2_in, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=v_embed, v0=v0)
            k2 = x2 - x2_in
            return x_in + k2, raw_v
        ...
        if mode == "rk4":
            ...
            x3_in = x_in + 0.5 * k2
            x3, _ = self._core_transition(x3_in, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=v_embed, v0=v0)
            k3 = x3 - x3_in

            x4_in = x_in + k3
            x4, _ = self._core_transition(x4_in, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=v_embed, v0=v0)
            k4 = x4 - x4_in
            return x_in + (k1 + 2.0 * k2 + 2.0 * k3 + k4) / 6.0, raw_v

        raise ValueError(f"Unknown integrator_mode={mode!r}")
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        transition_integrator: str = "euler",
        transition_hybrid_last_n: int = 4,
        longctx_taps_enabled: bool = False,
        longctx_tap_offsets: str = "64,256,1024",
        longctx_tap_prefix_mean: bool = True,
        longctx_tap_norm: bool = True,
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    integrator_mode="euler",
                    longctx_taps_enabled=longctx_taps_enabled,
                    longctx_tap_offsets=longctx_tap_offsets,
                    longctx_tap_prefix_mean=longctx_tap_prefix_mean,
                    longctx_tap_norm=longctx_tap_norm,
        ...
        if mode in ("hybrid_k2_k4", "k2_4_hybrid", "hybrid"):
            ...
            split_idx = num_layers - last_n
            for i, block in enumerate(self.blocks):
                block.integrator_mode = "rk4" if i >= split_idx else "rk2"
            self.transition_integrator_mode = f"hybrid_k2_k4(last_n={last_n})"
        elif mode in ("euler", "rk2", "rk4"):
            for block in self.blocks:
                block.integrator_mode = mode
            self.transition_integrator_mode = mode
        else:
            raise ValueError(
                "TRANSITION_INTEGRATOR must be one of: euler, rk2, rk4, hybrid_k2_k4"
            )
        if rope_dims > 0:
            head_dim = model_dim // num_heads
            for block in self.blocks:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        transition_integrator=args.transition_integrator,
        transition_hybrid_last_n=args.transition_hybrid_last_n,
        longctx_taps_enabled=args.longctx_taps_enabled,
        longctx_tap_offsets=args.longctx_tap_offsets,
        longctx_tap_prefix_mean=args.longctx_tap_prefix_mean,
        longctx_tap_norm=args.longctx_tap_norm,
    ...
    integrator_modes = [b.integrator_mode for b in base_model.blocks]
    rk2_layers = [i for i, mode in enumerate(integrator_modes) if mode == "rk2"]
    rk4_layers = [i for i, mode in enumerate(integrator_modes) if mode == "rk4"]
    log0(
        "transition_integrator:"
        f"{base_model.transition_integrator_mode} "
        f"rk2_layers:{rk2_layers} rk4_layers:{rk4_layers}"
    )
    tap_blocks = [i for i, b in enumerate(base_model.blocks) if b.longctx_taps_enabled and b.longctx_tap_scale is not None]
    if tap_blocks:
        tap_ref = base_model.blocks[tap_blocks[0]]
        log0(
            "longctx_taps:"
            f"enabled=1 blocks={tap_blocks} offsets:{list(tap_ref.longctx_tap_offsets)} "
            f"prefix_mean={int(tap_ref.longctx_prefix_logit is not None)} "
            f"norm={int(tap_ref.longctx_tap_norm)}"
        )
    else:
