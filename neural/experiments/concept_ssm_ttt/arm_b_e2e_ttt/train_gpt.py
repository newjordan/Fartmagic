...
class TTTFastMLP(nn.Module):
    """Tiny fast MLP for E2E TTT. Trained during pre-training, adapts at eval time."""
    def __init__(self, dim: int, hidden: int = 64):
        super().__init__()
        self.w1 = nn.Parameter(torch.zeros(hidden, dim))
    ...
    def forward(self, x: Tensor) -> Tensor:
        """Apply fast MLP as residual."""
        h = F.linear(x, self.w1)
        h = F.relu(h)
        h = F.linear(h, self.w2)
        return x + h * self.scale.to(dtype=x.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        # E2E TTT: fast MLPs on last 2 layers (training-time architecture, not eval hack)
        self.ttt_enabled = bool(int(os.environ.get("TTT_E2E", "0")))
        self.ttt_layers = nn.ModuleList()
        if self.ttt_enabled:
            ttt_hidden = int(os.environ.get("TTT_HIDDEN", "64"))
            ttt_layer_indices = [num_layers - 2, num_layers - 1]
            self.ttt_layer_indices = ttt_layer_indices
            for _ in ttt_layer_indices:
                self.ttt_layers.append(TTTFastMLP(model_dim, ttt_hidden))
        else:
            self.ttt_layer_indices = []
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
            if self.ttt_enabled and bi in self.ttt_layer_indices:
                ttt_idx = self.ttt_layer_indices.index(bi)
                x = self.ttt_layers[ttt_idx](x)
        x = self.final_norm(x)
