...
if _requested_compressor in ("zstd", "zstandard"):
    if zstandard is not None:
        ...
        _COMPRESSOR = "brotli"
        warnings.warn("zstandard not found — falling back to brotli")
    else:
        _COMPRESSOR = "zlib"
        warnings.warn("zstandard not found — falling back to zlib. Artifact may be larger.")
elif _requested_compressor == "brotli":
    if _brotli_module is not None:
        _COMPRESSOR = "brotli"
    elif zstandard is not None:
        _COMPRESSOR = "zstd"
        warnings.warn("brotli not found — falling back to zstd")
    else:
        _COMPRESSOR = "zlib"
        warnings.warn("brotli not found and zstandard unavailable — falling back to zlib")
elif _requested_compressor == "zlib":
    _COMPRESSOR = "zlib"
else:
    if zstandard is not None:
        _COMPRESSOR = "zstd"
        warnings.warn(f"unknown COMPRESSOR={_requested_compressor!r} — using zstd")
    elif _brotli_module is not None:
        _COMPRESSOR = "brotli"
        warnings.warn(f"unknown COMPRESSOR={_requested_compressor!r} — using brotli")
    else:
        _COMPRESSOR = "zlib"
        warnings.warn(f"unknown COMPRESSOR={_requested_compressor!r} — using zlib")
...
class Hyperparameters:
    ...
    gptq_online_enabled = bool(int(os.environ.get("GPTQ_ONLINE_ENABLED", "1")))
    gptq_online_start_frac = float(os.environ.get("GPTQ_ONLINE_START_FRAC", 0.72))
    gptq_online_lr_mul_max = float(os.environ.get("GPTQ_ONLINE_LR_MUL_MAX", 0.25))
    gptq_online_every = int(os.environ.get("GPTQ_ONLINE_EVERY", 4))
    gptq_online_first_micro_only = bool(int(os.environ.get("GPTQ_ONLINE_FIRST_MICRO_ONLY", "1")))
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def _flatten_hessian_input(x: Tensor) -> Tensor:
    x = x.detach().float()
    if x.ndim == 3:
        x = x.reshape(-1, x.shape[-1])
    return x
def _accum_gptq_hessian(hessians: dict[str, Tensor], n_seen: dict[str, int], name: str, x: Tensor) -> None:
    x2d = _flatten_hessian_input(x)
    if x2d.numel() == 0:
        return
    if name not in hessians:
        hessians[name] = torch.zeros(x2d.shape[1], x2d.shape[1], device=x2d.device, dtype=torch.float32)
        n_seen[name] = 0
    hessians[name].addmm_(x2d.t(), x2d)
    n_seen[name] += x2d.shape[0]
def _accum_gptq_bank_hessian(gptq_ctx: dict | None, bank_name: str, idx: int, x: Tensor) -> None:
    if gptq_ctx is None:
        return
    if not gptq_ctx.get("enabled", False):
        return
    if idx is None:
        return
    x2d = _flatten_hessian_input(x)
    if x2d.numel() == 0:
        return
    h_bank = gptq_ctx["hessians"][bank_name]
    n_bank = gptq_ctx["n_seen"][bank_name]
    h_bank[idx].addmm_(x2d.t(), x2d)
    n_bank[idx] += x2d.shape[0]
def init_online_gptq_ctx(model: nn.Module, device: torch.device) -> dict[str, object]:
    return {
        "enabled": False,
        "hessians": {
            "qo_bank": torch.zeros(model.qo_bank.shape[0], model.qo_bank.shape[2], model.qo_bank.shape[2], device=device, dtype=torch.float32),
            "kv_bank": torch.zeros(model.kv_bank.shape[0], model.kv_bank.shape[2], model.kv_bank.shape[2], device=device, dtype=torch.float32),
            "mlp_up_bank": torch.zeros(model.mlp_up_bank.shape[0], model.mlp_up_bank.shape[2], model.mlp_up_bank.shape[2], device=device, dtype=torch.float32),
            "mlp_down_bank": torch.zeros(model.mlp_down_bank.shape[0], model.mlp_down_bank.shape[2], model.mlp_down_bank.shape[2], device=device, dtype=torch.float32),
        },
        "n_seen": {
            "qo_bank": torch.zeros(model.qo_bank.shape[0], device=device, dtype=torch.int64),
            "kv_bank": torch.zeros(model.kv_bank.shape[0], device=device, dtype=torch.int64),
            "mlp_up_bank": torch.zeros(model.mlp_up_bank.shape[0], device=device, dtype=torch.int64),
            "mlp_down_bank": torch.zeros(model.mlp_down_bank.shape[0], device=device, dtype=torch.int64),
        },
    }
def finalize_online_gptq_hessians(gptq_ctx: dict[str, object], distributed: bool) -> tuple[dict[str, Tensor], dict[str, Tensor]]:
    out_hessians: dict[str, Tensor] = {}
    out_counts: dict[str, Tensor] = {}
    hessians = gptq_ctx["hessians"]
    n_seen = gptq_ctx["n_seen"]
    for name in ("qo_bank", "kv_bank", "mlp_up_bank", "mlp_down_bank"):
        h_bank = hessians[name]
        n_bank = n_seen[name]
        if distributed:
            dist.all_reduce(h_bank, op=dist.ReduceOp.SUM)
            dist.all_reduce(n_bank, op=dist.ReduceOp.SUM)
        valid = n_bank > 0
        if not bool(valid.any().item()):
            continue
        denom = n_bank.clamp_min(1).to(dtype=h_bank.dtype)
        out_hessians[name] = (h_bank / denom[:, None, None]).cpu()
        out_counts[name] = n_bank.clone().cpu()
    return out_hessians, out_counts
...
def gptq_calibrate(model: nn.Module, train_pattern: str, device: torch.device,
    ...
    def make_hook(name: str):
        def hook_fn(module, inp, out):
            _accum_gptq_hessian(hessians, n_seen, name, inp[0])
        return hook_fn
    ...
    for name, h_bank in bank_ctx["hessians"].items():
        counts = bank_ctx["n_seen"][name].clamp_min_(1).to(dtype=h_bank.dtype)
        hessians[name] = h_bank / counts[:, None, None]
    model.train()
    return hessians
...
def mixed_quantize_gptq(state_dict: dict[str, Tensor], bit_policy: dict[str, int],
    ...
    for name, tensor in state_dict.items():
        ...
        if t.ndim == 2:
            ...
            H_bank = hessians.get(name)
            q_slices: list[Tensor] = []
            s_slices: list[Tensor] = []
            used_gptq = (
                H_bank is not None
                and H_bank.ndim == 3
                and H_bank.shape[0] == t.shape[0]
                and H_bank.shape[1] == t.shape[2]
            )
            for i in range(t.shape[0]):
                if used_gptq:
                    q_i, s_i = gptq_quantize_weight(t[i], H_bank[i].cpu(), clip_range=clip_range)
                    gptq_count += 1
                else:
                    q_i, s_i = quantize_lowbit_per_row(t[i], bits)
                    naive_count += 1
                q_slices.append(q_i)
                s_slices.append(s_i)
            result[name + ".q"] = torch.stack(q_slices, dim=0)
            result[name + ".scale"] = torch.stack(s_slices, dim=0)
            meta[name] = {"type": f"int{bits}", "bits": bits}
            quant_counts[bits] = quant_counts.get(bits, 0) + 1
        elif t.ndim >= 1:
    ...
    print(f"mixed_quantize: {gptq_count} GPTQ tensors, {naive_count} naive tensors {quant_summary}".strip(), flush=True)
    return result, meta
def dequantize_mixed_quant(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            ...
            val = (q.float() * scale).to(orig_dtype)
        else:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, *, q_idx: int | None = None, k_idx: int | None = None, v_idx: int | None = None, out_idx: int | None = None, gptq_ctx: dict | None = None) -> tuple[Tensor, Tensor | None]:
...
class Block(nn.Module):
    def __init__(
        ...
        self.parallel = False  # set True externally for GPT-J style parallel attn+MLP
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, *, q_idx: int | None = None, k_idx: int | None = None, v_idx: int | None = None, out_idx: int | None = None, up_idx: int | None = None, down_idx: int | None = None, gptq_ctx: dict | None = None) -> tuple[Tensor, Tensor | None]:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_in = self.attn_norm(x_in) * self.ln_scale_factor
        attn_out, raw_v = self.attn(attn_in, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0, q_idx=q_idx, k_idx=k_idx, v_idx=v_idx, out_idx=out_idx, gptq_ctx=gptq_ctx)
        if self.parallel:
            mlp_in = self.mlp_norm(x_in) * self.ln_scale_factor
            mlp_out = self.mlp(mlp_in, up_w, down_w, up_idx=up_idx, down_idx=down_idx, gptq_ctx=gptq_ctx)
            ...
            x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(mlp_in, up_w, down_w, up_idx=up_idx, down_idx=down_idx, gptq_ctx=gptq_ctx)
...
class GPT(nn.Module):
    def __init__(
        ...
        if _num_loops > 0:
            ...
            _loop_end = int(os.environ.get("LOOP_END", "5"))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for idx in enc_iter:
            ...
            x, raw_v = self.blocks[idx](x, x0,
                ...
                v_embed=ve, v0=v0,
                q_idx=idx, k_idx=idx, v_idx=n + idx, out_idx=n + idx,
                up_idx=idx, down_idx=idx, gptq_ctx=gptq_ctx)
...
def main() -> None:
    ...
    if args.init_model_path:
        log0(f"init_model:loading path={args.init_model_path}")
        init_state = torch.load(args.init_model_path, map_location="cpu")
        if isinstance(init_state, dict) and "state_dict" in init_state and isinstance(init_state["state_dict"], dict):
            init_state = init_state["state_dict"]
        elif isinstance(init_state, dict) and "model" in init_state and isinstance(init_state["model"], dict):
            init_state = init_state["model"]
    ...
    _skip_gptq = bool(int(os.environ.get("SKIP_GPTQ", "0")))
    _needs_lowbit_gptq = any(bits < 8 for bits in quant_policy.values())
    _use_online_gptq = args.gptq_online_enabled and (not _skip_gptq) and _needs_lowbit_gptq
    ...
    online_gptq_ctx: dict[str, object] | None = None
    online_gptq_collect_steps = 0
    online_gptq_collect_micro = 0
    online_gptq_started = False
    if _use_online_gptq:
        online_gptq_ctx = init_online_gptq_ctx(base_model, device)
        base_model._gptq_ctx = online_gptq_ctx
        log0(
            "gptq_online:enabled "
            f"start_frac:{args.gptq_online_start_frac:.2f} "
            f"lr_mul_max:{args.gptq_online_lr_mul_max:.3f} "
            f"every:{max(args.gptq_online_every, 1)} "
            f"first_micro_only:{int(args.gptq_online_first_micro_only)}"
        )
    else:
        log0(f"gptq_online:disabled skip_gptq:{int(_skip_gptq)} needs_lowbit:{int(_needs_lowbit_gptq)}")
    ema_decay = 0.997
    ...
    while True:
        ...
        progress_frac = step / max(args.iterations, 1) if max_wallclock_ms is None else elapsed_ms / max(max_wallclock_ms, 1e-9)
        if args.num_loops > 0 and not base_model.looping_active:
            if progress_frac >= args.enable_looping_at:
                base_model.looping_active = True
                log0(f"layer_loop:enabled step:{step} frac:{progress_frac:.3f} encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
        scale = lr_mul(step, elapsed_ms)
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        collect_gptq_this_step = False
        if online_gptq_ctx is not None:
            if args.warmdown_iters <= 0:
                warmdown_started = False
            elif max_wallclock_ms is None:
                warmdown_start = max(args.iterations - args.warmdown_iters, 0)
                warmdown_started = step >= warmdown_start
            else:
                warmdown_started = scale < 1.0
            collect_gptq_this_step = (
                base_model.looping_active
                and warmdown_started
                and scale < args.gptq_online_lr_mul_max
                and progress_frac >= args.gptq_online_start_frac
                and (step % max(args.gptq_online_every, 1) == 0)
            )
            if collect_gptq_this_step and not online_gptq_started:
                online_gptq_started = True
                log0(
                    f"gptq_online:start step:{step} frac:{progress_frac:.3f} "
                    f"scale:{scale:.4f} looping:{int(base_model.looping_active)}"
                )
            if collect_gptq_this_step:
                online_gptq_collect_steps += 1
            online_gptq_ctx["enabled"] = False
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            collect_this_micro = collect_gptq_this_step and (
                (not args.gptq_online_first_micro_only) or micro_step == 0
            )
            if online_gptq_ctx is not None:
                online_gptq_ctx["enabled"] = bool(collect_this_micro)
            if collect_this_micro:
                online_gptq_collect_micro += 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
        ...
        muon_frac = min(step / args.muon_momentum_warmup_steps, 1.0) if args.muon_momentum_warmup_steps > 0 else 1.0
        muon_momentum = (1 - muon_frac) * args.muon_momentum_warmup_start + muon_frac * args.muon_momentum
        for group in optimizer_muon.param_groups:
    ...
    if _skip_gptq:
        ...
        gptq_hessians: dict[str, Tensor] = {}
    elif not args.gptq_online_enabled:
        log0("gptq:online disabled (GPTQ_ONLINE_ENABLED=0); using no Hessians")
        gptq_hessians = {}
    else:
        t_gptq = time.perf_counter()
        if online_gptq_ctx is None:
            log0("gptq:online context missing; using no Hessians")
            gptq_hessians = {}
        else:
            gptq_hessians, gptq_counts = finalize_online_gptq_hessians(online_gptq_ctx, distributed)
            total_samples = sum(int(v.sum().item()) for v in gptq_counts.values()) if gptq_counts else 0
            if total_samples <= 0 or not gptq_hessians:
                gptq_hessians = {}
                log0(
                    f"gptq:online finalized 0 samples (steps:{online_gptq_collect_steps} "
                    f"micro:{online_gptq_collect_micro}); quantizer will fallback to naive"
                )
            else:
                bank_summary = " ".join(
                    f"{name}:{int(v.sum().item())}"
                    for name, v in sorted(gptq_counts.items())
                )
                log0(
                    f"gptq:online finalized {len(gptq_hessians)} banks samples:{total_samples} "
    ...
    if _COMPRESSOR == "zstd":
        ...
        quant_payload = _zlib_module.decompress(quant_blob_disk)
    quant_state = torch.load(io.BytesIO(quant_payload), map_location="cpu")
    deq_state = dequantize_mixed_quant(quant_state["w"], quant_state["m"], sd_cpu)
    eval_model = GPT(
    ...
    if args.skip_final_eval:
        ...
        if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
            ...
            sw_val_loss, sw_val_bpb = eval_val_sliding(
                args, deployed_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
