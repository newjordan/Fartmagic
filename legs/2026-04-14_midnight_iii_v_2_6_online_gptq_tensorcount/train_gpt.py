...
def _accumulate_hessian(
    hessians: dict[str, Tensor], n_seen: dict[str, int], name: str, x: Tensor
) -> None:
    x = x.detach().float()
    if x.ndim >= 3:
...
class OnlineGptqAccumulator:
    """Accumulate GPTQ Hessians during normal training forwards."""

    def __init__(self, mode: str, every_steps: int = 4, max_rows: int = 4096):
        self.mode = mode
        self.every_steps = max(1, every_steps)
        self.max_rows = max(0, max_rows)
        self.hessian_sums: dict[str, Tensor] = {}
        self.n_seen: dict[str, Tensor] = {}
        self._enabled = False

    def set_enabled(self, enabled: bool) -> None:
        self._enabled = enabled

    def should_collect(self, step: int, looping_active: bool, require_looping: bool) -> bool:
        if step % self.every_steps != 0:
            return False
        if require_looping and not looping_active:
            return False
        return True

    def _prepare_x(self, x: Tensor) -> Tensor | None:
        x = x.detach()
        if x.ndim >= 3:
            x = x.reshape(-1, x.shape[-1])
        elif x.ndim == 1:
        ...
        if x.ndim != 2 or x.numel() == 0:
            return None
        if self.max_rows > 0 and x.shape[0] > self.max_rows:
            stride = max(1, x.shape[0] // self.max_rows)
            x = x[::stride][: self.max_rows]
        x = x.float()
        if self.mode == "online_cpu":
            x = x.cpu()
        return x

    def collect(self, name: str, x: Tensor) -> None:
        if not self._enabled:
            return
        x2 = self._prepare_x(x)
        if x2 is None:
            return
        if name not in self.hessian_sums:
            self.hessian_sums[name] = torch.zeros(
                x2.shape[1], x2.shape[1], device=x2.device, dtype=torch.float32
            )
            self.n_seen[name] = torch.zeros((), device=x2.device, dtype=torch.float32)
        self.hessian_sums[name].addmm_(x2.t(), x2)
        self.n_seen[name].add_(float(x2.shape[0]))

    def finalize(self, device: torch.device, distributed: bool) -> dict[str, Tensor]:
        if distributed:
            for name in sorted(self.hessian_sums):
                H = self.hessian_sums[name]
                if H.device.type == "cuda":
                    dist.all_reduce(H, op=dist.ReduceOp.SUM)
                else:
                    H_gpu = H.to(device=device, dtype=torch.float32)
                    dist.all_reduce(H_gpu, op=dist.ReduceOp.SUM)
                    H = H_gpu.cpu()
                count_tensor = self.n_seen.get(name)
                if count_tensor is None:
                    continue
                if count_tensor.device.type == "cuda":
                    dist.all_reduce(count_tensor, op=dist.ReduceOp.SUM)
                else:
                    count_gpu = count_tensor.to(device=device, dtype=torch.float32)
                    dist.all_reduce(count_gpu, op=dist.ReduceOp.SUM)
                    count_tensor = count_gpu.cpu()
                self.hessian_sums[name] = H
                self.n_seen[name] = count_tensor
        out: dict[str, Tensor] = {}
        for name, H in self.hessian_sums.items():
            seen_tensor = self.n_seen.get(name)
            if seen_tensor is None:
                continue
            seen = float(seen_tensor.item())
            if seen <= 0:
                continue
            out[name] = (H / seen).to(dtype=torch.float32, device="cpu")
        return out
...
def gptq_calibrate(model: nn.Module, train_pattern: str, device: torch.device,
    ...
    """Collect Hessian H = X^T X for linear and bank-backed projections."""
    ...
    def make_hook(name: str):
        def hook_fn(module, inp, out):
            _accumulate_hessian(hessians, n_seen, name, inp[0])
        return hook_fn
    ...
    def bank_collector(name: str, x: Tensor) -> None:
        _accumulate_hessian(hessians, n_seen, name, x)

    prev_collector = getattr(model, "_gptq_collector", None)
    setattr(model, "_gptq_collector", bank_collector)
    ...
    try:
        ...
        for h in hooks:
            h.remove()
        setattr(model, "_gptq_collector", prev_collector)
...
def mixed_quantize_gptq(state_dict: dict[str, Tensor], bit_policy: dict[str, int],
    ...
    for name, tensor in state_dict.items():
        ...
        if t.ndim == 2:
            ...
            t_3d = t.reshape(-1, t.shape[-2], t.shape[-1]) if t.ndim > 3 else t
            q_slices: list[Tensor] = []
            s_slices: list[Tensor] = []
            for idx in range(t_3d.shape[0]):
                w = t_3d[idx]
                H = hessians.get(f"{name}.{idx}")
                if H is not None and H.shape[0] == w.shape[1]:
                    q_i, s_i = gptq_quantize_weight(w, H.cpu(), clip_range=clip_range)
                    gptq_count += 1
                else:
                    q_i, s_i = quantize_lowbit_per_row(w, bits)
                    naive_count += 1
                q_slices.append(q_i)
                s_slices.append(s_i)
            q = torch.cat(q_slices, dim=0)
            s = torch.cat(s_slices, dim=0)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": f"int{bits}", "bits": bits}
            quant_counts[bits] = quant_counts.get(bits, 0) + 1
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if gptq_collector is not None:
            if q_name is not None:
                gptq_collector(q_name, x)
            if k_name is not None:
                gptq_collector(k_name, x)
            if v_name is not None:
                gptq_collector(v_name, x)
        ...
        if gptq_collector is not None and out_name is not None:
            gptq_collector(out_name, y)
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        if gptq_collector is not None and up_name is not None:
            gptq_collector(up_name, x)
        x = F.linear(x, up_w.to(x.dtype))
        x = leaky_relu_sq(x, kernel_mode=self.kernel_mode)
        if gptq_collector is not None and down_name is not None:
            gptq_collector(down_name, x)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        gptq_collector = getattr(self, "_gptq_collector", None)
        ...
        for idx in enc_iter:
            ...
            x, raw_v = self.blocks[idx](
                ...
                self.kv_bank[n + idx],
                self.qo_bank[n + idx],
                self.mlp_up_bank[idx],
                self.mlp_down_bank[idx],
                v_embed=ve,
                v0=v0,
                gptq_collector=gptq_collector,
                q_name=f"qo_bank.{idx}",
                k_name=f"kv_bank.{idx}",
                v_name=f"kv_bank.{n + idx}",
                out_name=f"qo_bank.{n + idx}",
                up_name=f"mlp_up_bank.{idx}",
                down_name=f"mlp_down_bank.{idx}",
            )
...
def main() -> None:
    ...
    _gptq_cal_mode = os.environ.get("GPTQ_CAL_MODE", "offline").strip().lower()
    if _gptq_cal_mode == "online":
        _gptq_cal_mode = "online_gpu"
    if _gptq_cal_mode not in {"offline", "online_gpu", "online_cpu"}:
        log0(f"gptq:unknown GPTQ_CAL_MODE={_gptq_cal_mode!r} -> using offline")
        _gptq_cal_mode = "offline"
    _gptq_online_every = max(1, int(os.environ.get("GPTQ_ONLINE_EVERY", "4")))
    _gptq_online_max_rows = max(0, int(os.environ.get("GPTQ_ONLINE_MAX_ROWS", "4096")))
    _gptq_online_require_looping = bool(int(os.environ.get("GPTQ_ONLINE_REQUIRE_LOOPING", "1")))
    online_gptq = None
    if _gptq_cal_mode != "offline" and not _skip_gptq and _needs_lowbit_gptq:
        online_gptq = OnlineGptqAccumulator(
            mode=_gptq_cal_mode,
            every_steps=_gptq_online_every,
            max_rows=_gptq_online_max_rows,
        )
        online_gptq.set_enabled(False)
        setattr(base_model, "_gptq_collector", online_gptq.collect)
        log0(
            f"gptq:online mode:{_gptq_cal_mode} every_steps:{_gptq_online_every} "
            f"max_rows:{_gptq_online_max_rows} require_looping:{int(_gptq_online_require_looping)}"
        )
    else:
        setattr(base_model, "_gptq_collector", None)
    ...
    while True:
        ...
        collect_online_gptq = (
            online_gptq is not None
            and online_gptq.should_collect(step, base_model.looping_active, _gptq_online_require_looping)
        )
        ...
        for micro_step in range(grad_accum_steps):
            if online_gptq is not None:
                online_gptq.set_enabled(collect_online_gptq)
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        base_model.load_state_dict(avg_state, strict=True)
    # GPTQ calibration runs after EMA/LAWA so Hessians match the exact exported weights.
    if _skip_gptq:
        ...
        if online_gptq is not None:
            ...
            gptq_hessians = online_gptq.finalize(device=device, distributed=distributed)
            dt = time.perf_counter() - t_gptq
            if len(gptq_hessians) > 0:
                log0(f"gptq:calibrated {len(gptq_hessians)} layers in {dt:.1f}s (online)")
            else:
                log0("gptq:online_empty -> fallback_offline_calibration")
        if len(gptq_hessians) == 0:
