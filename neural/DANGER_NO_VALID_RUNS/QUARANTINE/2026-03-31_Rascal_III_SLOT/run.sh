...
cuda_ver=$("${PYTHON_BIN}" -c "import torch; print(torch.version.cuda or 'NONE')" 2>/dev/null) \
    || die "python/torch failed"
...
SLOT_ENABLED=1 \
"${PYTHON_BIN}" -m torch.distributed.run --standalone --nproc_per_node="${NPROC}" "${SRC}" \
2>&1 | tee "${LOG}"
