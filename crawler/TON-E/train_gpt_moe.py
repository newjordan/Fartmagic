...
class Hyperparameters:
    ...
    xsa_include_flat = bool(int(os.environ.get("XSA_INCLUDE_FLAT", "0")))
    ...
    crawler_moe = bool(int(os.environ.get("CRAWLER_MOE", "0")))
    crawler_moe_hidden = int(os.environ.get("CRAWLER_MOE_HIDDEN", "16"))
...
def apply_ton_e_rhythm_profile(args: Hyperparameters) -> None:
    ...
    if "CRAWLER_LOOPS" not in os.environ:
        args.crawler_loops = int(os.environ.get("TON_E_CRAWLER_LOOPS", "2"))
    if "XSA_INCLUDE_FLAT" not in os.environ:
        args.xsa_include_flat = bool(int(os.environ.get("TON_E_XSA_INCLUDE_FLAT", "1")))
    if "XSA_LAST_N" not in os.environ:
        stack_depth = max(args.num_flat_layers + args.num_crawler_layers, 1)
        args.xsa_last_n = int(os.environ.get("TON_E_XSA_LAST_N", str(stack_depth)))
    if "CRAWLER_LOOP_ROPE_SCALES" not in os.environ and "TON_E_CRAWLER_LOOP_ROPE_SCALES" in os.environ:
        args.crawler_loop_rope_scales = _parse_int_tuple(
            os.environ["TON_E_CRAWLER_LOOP_ROPE_SCALES"],
            args.crawler_loop_rope_scales,
        )
    if len(args.crawler_loop_rope_scales) < args.crawler_loops:
        tail = args.crawler_loop_rope_scales[-1] if args.crawler_loop_rope_scales else 1
        args.crawler_loop_rope_scales = args.crawler_loop_rope_scales + (tail,) * (
            args.crawler_loops - len(args.crawler_loop_rope_scales)
        )
...
class CrawlerMoEGate(nn.Module):
    """Lightweight MoE gate for crawler loops.

    Per-token, per-loop soft weighting of each crawler block's contribution.
    The gate learns WHEN each shared block matters more — not different expert
    weights, since crawler blocks share parameters across loops.

    Architecture:
      - Shared bottleneck projection: model_dim -> moe_hidden  (tiny, e.g. 16)
      - Per-loop-per-block projection: moe_hidden -> 1  (one scalar gate per block)
      - Per-loop-per-block bias: learned scalar offset
      - Output: sigmoid(proj(bottleneck(x)) + bias)  in [0, 1] per token per block

    Total params for 3 loops x 3 blocks, moe_hidden=16:
      - bottleneck: 512 * 16 = 8192  (shared across loops/blocks)
      - per-loop-block proj: 16 * 9 = 144
      - per-loop-block bias: 9
      - Total: ~8.3K params — negligible in 16MB budget

    Init: biases at +2.0 -> sigmoid(2)≈0.88 -> blocks contribute ~88% at start.
    This is a warm start: the model begins near-identical to the unmodified crawler,
    then learns to modulate block importance per token and per loop.
    """
    def __init__(self, model_dim: int, num_crawler_layers: int, crawler_loops: int,
                 moe_hidden: int = 16):
        super().__init__()
        self.num_crawler_layers = num_crawler_layers
        self.crawler_loops = crawler_loops
        self.moe_hidden = moe_hidden
        # Shared bottleneck: model_dim -> moe_hidden (no bias, keeps it tiny)
        self.proj_down = nn.Linear(model_dim, moe_hidden, bias=False)
        nn.init.normal_(self.proj_down.weight, std=0.01)
        # Per-loop-per-block: moe_hidden -> 1 scalar
        # Stored as [crawler_loops, num_crawler_layers, moe_hidden]
        self.gate_proj = nn.Parameter(
            torch.randn(crawler_loops, num_crawler_layers, moe_hidden) * 0.01
        )
        # Per-loop-per-block bias (init +2.0 -> sigmoid ≈ 0.88, warm start)
        self.gate_bias = nn.Parameter(
            torch.full((crawler_loops, num_crawler_layers), 2.0, dtype=torch.float32)
        )

    def forward(self, x: Tensor, loop_idx: int) -> Tensor:
        """Compute per-token gate weights for each crawler block in this loop.

        Args:
            x: hidden state [B, T, model_dim]
            loop_idx: which crawler loop (0-indexed)

        Returns:
            gate_weights: [B, T, num_crawler_layers] in (0, 1)
        """
        # Project to bottleneck: [B, T, moe_hidden]
        h = self.proj_down(x.detach())  # detach: gate gradient doesn't flow back into main hidden state
        # Compute per-block logits: einsum [B, T, moe_hidden] x [num_blocks, moe_hidden] -> [B, T, num_blocks]
        proj_w = self.gate_proj[loop_idx]  # [num_blocks, moe_hidden]
        bias = self.gate_bias[loop_idx]    # [num_blocks]
        logits = torch.einsum('bth,nh->btn', h.to(proj_w.dtype), proj_w) + bias[None, None, :]
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None,
                loop_idx: int | None = None, cos_sin: tuple | None = None) -> Tensor:
...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        fused_norm: bool = False,
        crawler_moe: bool = False,
        crawler_moe_hidden: int = 16,
    ):
        super().__init__()
        ...
        # MoE gate: per-token soft weighting of each crawler block's contribution per loop.
        self.crawler_moe_gate = (
            CrawlerMoEGate(model_dim, num_crawler_layers, crawler_loops, moe_hidden=crawler_moe_hidden)
            if (crawler_moe and num_crawler_layers > 0 and crawler_loops > 0)
            else None
        )
    ...
    def _run_crawler(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict,
        ...
        for loop in range(active_loops):
            ...
            # MoE gate: compute per-token per-block weights for this loop iteration
            moe_weights = None
            if self.crawler_moe_gate is not None:
                moe_weights = self.crawler_moe_gate(x_loop, loop)  # [B, T, num_crawler_layers]
            for ci in range(active_layers):
                ...
                if moe_weights is not None:
                    x_before_block = x_loop
                x_loop = block(x_loop, x0, v_embed=ve, loop_idx=loop, cos_sin=lcs)
                # MoE: scale this block's residual contribution by its gate weight
                if moe_weights is not None:
                    g = moe_weights[:, :, ci].unsqueeze(-1)  # [B, T, 1]
                    x_loop = x_before_block + g * (x_loop - x_before_block)
...
def build_model(args: Hyperparameters, device: torch.device) -> nn.Module:
    ...
    if args.use_crawler:
        model = CrawlerGPT(
            ...
            xsa_include_flat=args.xsa_include_flat,
            ...
            crawler_moe=args.crawler_moe,
            crawler_moe_hidden=args.crawler_moe_hidden,
        )
    else:
        model = GPT(
            vocab_size=args.vocab_size,
...
def main() -> None:
    ...
    # MoE gate params: all go to AdamW scalar optimizer (tiny module, not suited for Muon).
    if hasattr(base_model, 'crawler_moe_gate') and base_model.crawler_moe_gate is not None:
        moe_gate = base_model.crawler_moe_gate
        scalar_params.append(moe_gate.proj_down.weight)    # [moe_hidden, model_dim] tiny 2D
        scalar_params.append(moe_gate.gate_proj)           # [loops, blocks, moe_hidden] 3D
        scalar_params.append(moe_gate.gate_bias)           # [loops, blocks] 2D control tensor
    ...
    moe_gate_params = 0
    if hasattr(base_model, 'crawler_moe_gate') and base_model.crawler_moe_gate is not None:
        moe_gate_params = sum(p.numel() for p in base_model.crawler_moe_gate.parameters())
    log0(f"crawler_moe:{int(args.crawler_moe)} moe_hidden:{args.crawler_moe_hidden} moe_gate_params:{moe_gate_params}")
    log0(
    ...
    if master_process:
        submission_total_bytes = quant_file_bytes + code_bytes
        ...
        log0(f"  steps:         {step}")
        log0(f"  train_time_s:  {int(round(training_time_ms / 1000.0))}")
        log0(f"  bytes_total:   {submission_total_bytes}  (limit 16000000)")
        log0(f"  bytes_code:    {code_bytes}")
        log0(f"  artifact_legal:{'yes' if artifact_legal else 'no'}")
        log0("============================================")
    if distributed:
