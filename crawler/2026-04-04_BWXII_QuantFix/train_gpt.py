...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        self._smart_skip: bool = False
        # Smart Skip: crawler confidence head gates each skip connection
        self.skip_confidence = nn.Linear(model_dim, 1, bias=True)
        nn.init.zeros_(self.skip_confidence.weight)
        nn.init.constant_(self.skip_confidence.bias, 2.0)  # sigmoid(2)≈0.88 — starts near pass-through
        self.final_norm = RMSNorm()
        self.lm_head = None if tie_embeddings else CastedLinear(model_dim, vocab_size, bias=False)
        if self.lm_head is not None:
            self.lm_head._zero_init = True
...
def main() -> None:
    ...
    if hasattr(base_model, '_cross_heads'):
        ...
        base_model._smart_skip = args.smart_skip
        if args.crawler_cross_heads > 0:
            log0(f"splithead:cross_heads={args.crawler_cross_heads} v0_residual={args.crawler_v0_residual} smart_skip={args.smart_skip}")
