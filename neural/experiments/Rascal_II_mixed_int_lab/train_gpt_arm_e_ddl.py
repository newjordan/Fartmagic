...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
        # DDL: Deep Delta Learning residual (arXiv 2601.00417)
        # Rank-1 delta operator: x' = x + beta * k * (k^T * f(x) - k^T * x)
        # k = learnable unit direction per layer, beta = scalar gate in (0, 2)
        self.ddl_enabled = bool(int(os.environ.get("DDL_ENABLED", "0")))
        if self.ddl_enabled:
            self.ddl_k = nn.Parameter(torch.randn(dim) / math.sqrt(dim))
            self.ddl_beta = nn.Parameter(torch.tensor(1.0, dtype=torch.float32))
    def _ddl_residual(self, x_in: Tensor, x_out: Tensor) -> Tensor:
        """Apply DDL: project residual update onto learnable direction k,
        with erasure of the x_in component along k. beta in (0,2) via sigmoid*2."""
        k = F.normalize(self.ddl_k.to(dtype=x_in.dtype), dim=0)  # unit direction
        beta = 2.0 * torch.sigmoid(self.ddl_beta.to(dtype=x_in.dtype))  # gate in (0, 2)
        delta = x_out - x_in  # standard residual update
        # Project: keep standard update + DDL erasure/injection along k
        k_dot_delta = (delta * k).sum(dim=-1, keepdim=True)  # [B, T, 1]
        k_dot_x = (x_in * k).sum(dim=-1, keepdim=True)       # [B, T, 1]
        # DDL update: erase x_in component along k, inject delta component along k
        ddl_update = beta * k * (k_dot_delta - k_dot_x) + delta
        return x_in + ddl_update
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.ddl_enabled:
            x_out = self._ddl_residual(x_in, x_out)
        return x_out, raw_v
