...
TRAIN_SCRIPT="${REPO_ROOT}/legs/2026-04-12_midnight_iii_v_submission/train_gpt.py"
TRACKED_ENV="${REPO_ROOT}/legs/2026-04-12_midnight_iii_v_submission/tracked_env.sh"
LOG_DIR="${REPO_ROOT}/legs/2026-04-12_midnight_iii_v_submission/logs"
mkdir -p "${LOG_DIR}"
...
# LEG_DIFF_GUARD_ARGS="--max-code-changes 80 --max-total-changed-lines 120" bash ${REPO_ROOT}/legs/2026-04-12_midnight_iii_v_submission/gate.sh
if [[ -n "${LEG_DIFF_GUARD_ARGS:-}" ]]; then
  # shellcheck disable=SC2086
  python3 "${REPO_ROOT}/scripts/leg_diff_guard.py" "${REPO_ROOT}/legs/2026-04-12_midnight_iii_v_submission" ${LEG_DIFF_GUARD_ARGS}
else
  python3 "${REPO_ROOT}/scripts/leg_diff_guard.py" "${REPO_ROOT}/legs/2026-04-12_midnight_iii_v_submission"
fi
