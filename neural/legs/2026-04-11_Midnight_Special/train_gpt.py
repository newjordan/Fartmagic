...
def eval_val_ttt(
    ...
    """Score-first TTT: score each chunk under no_grad, then SGD update before next chunk.
    Legal per Issue #1017: each token scored once, prefix-only, no rescoring."""
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    ttt_chunk = args.ttt_chunk_tokens
    context_size = seq_len - stride
    # Group windows by chunk for TTT
    ...
    for ci in range(num_chunks):
        ...
        my_e = len(windows) * (rank + 1) // world_size
        my_windows = windows[my_s:my_e]
        # Score phase: no_grad, score all windows in this chunk
        base_model.eval()
        with torch.no_grad():
            for bi in range(0, len(my_windows), batch_seqs):
                batch_ws = my_windows[bi:bi + batch_seqs]
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()
        # Update phase: SGD on this chunk's data (skip last chunk)
...
def main() -> None:
    ...
    if args.skip_final_eval:
        ...
        if args.ttt_enabled:
            # Deserialize fresh quantized model for TTT (TTT modifies weights)
            torch._dynamo.reset()
            torch.cuda.empty_cache()
            with open(args.quant_artifact_path, "rb") as f:
                ttt_blob = f.read()
            if _COMPRESSOR == "zstd":
                ttt_payload = zstandard.ZstdDecompressor().decompress(ttt_blob)
            elif _COMPRESSOR == "brotli":
                ttt_payload = _brotli_module.decompress(ttt_blob)
            else:
                ttt_payload = _zlib_module.decompress(ttt_blob)
            ttt_qs = torch.load(io.BytesIO(ttt_payload), map_location="cpu")
            ttt_deq = dequantize_mixed_quant(ttt_qs["w"], ttt_qs["m"], export_sd)
            ttt_model = GPT(
                vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim,
                num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult,
                tie_embeddings=args.tie_embeddings, tied_embed_init_std=args.tied_embed_init_std,
            ...
            ttt_model.load_state_dict(ttt_deq, strict=True)
            if args.num_loops > 0:
            ...
            del ttt_model, ttt_deq, ttt_qs, ttt_payload, ttt_blob
            torch.cuda.empty_cache()
