...
class Hyperparameters:
    ...
    export_model_path = os.environ.get("EXPORT_MODEL_PATH", "./artifacts/midnight_anchor/final_model_anchor.pt")
...
def main() -> None:
    ...
    if _skip_gptq:
        ...
        if master_process:
            export_path = args.export_model_path
            os.makedirs(os.path.dirname(export_path), exist_ok=True)
            torch.save(export_sd, export_path)
            model_bytes = os.path.getsize(export_path)
            code_bytes = len(code.encode("utf-8"))
            log0(f"anchor_model_path:{export_path}")
            log0(f"Serialized model: {model_bytes} bytes")
            log0(f"Code size: {code_bytes} bytes")
        if distributed:
            dist.barrier()
            dist.destroy_process_group()
        log0("anchor:modelbuild_complete export_only=1")
        return
    full_state_dict = base_model.state_dict()
    export_sd = {k: v for k, v in full_state_dict.items() if "mtp_heads" not in k}
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_heads" in k)
