...
class SimpleSSM(nn.Module):
    """Minimal diagonal state-space model replacing attention.
    Linear-time in sequence length. State carries context forward.
    NOTE: The sequential scan is slow in pure PyTorch. This is a concept
    demonstration — a real implementation would use a parallel scan kernel."""
    def __init__(self, dim: int, state_dim: int = 64):
        super().__init__()
        self.state_dim = state_dim
        # Input projection
        self.in_proj = CastedLinear(dim, state_dim * 2, bias=False)  # x -> (B_input, C_input)
        # Diagonal state transition (log-parameterized for stability)
        self.log_A = nn.Parameter(torch.randn(state_dim) * 0.01 - 4.0)  # init near -4 -> A ≈ 0.018
        # Output projection
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Project input
        bc = self.in_proj(x)  # (bsz, seq_len, state_dim * 2)
        B_input, C_input = bc.chunk(2, dim=-1)  # each (bsz, seq_len, state_dim)

        # Diagonal state transition
        A = -torch.exp(self.log_A).to(dtype=x.dtype)  # (state_dim,) negative for stability

        # Scan: h[t] = A * h[t-1] + B_input[t], y[t] = C_input[t] * h[t]
        # Sequential scan (correct but slow — concept PR, not production)
        h = torch.zeros(bsz, self.state_dim, device=x.device, dtype=x.dtype)
        outputs = []
        for t in range(seq_len):
            h = A * h + B_input[:, t, :]
            outputs.append(h)
        h_all = torch.stack(outputs, dim=1)  # (bsz, seq_len, state_dim)

        # Output
        y = self.out_proj(h_all * C_input)
        return x + y * self.scale.to(dtype=x.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        # SSM hybrid: replace attention in first N layers with SSM
        self.ssm_hybrid = bool(int(os.environ.get("SSM_HYBRID", "0")))
        self.ssm_layers_n = int(os.environ.get("SSM_LAYERS", "4"))
        self.ssm_modules = nn.ModuleList()
        if self.ssm_hybrid:
            ssm_state_dim = int(os.environ.get("SSM_STATE_DIM", "64"))
            for _ in range(self.ssm_layers_n):
                self.ssm_modules.append(SimpleSSM(model_dim, ssm_state_dim))
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            if self.ssm_hybrid and i < self.ssm_layers_n:
                # SSM replaces attention, keep MLP
                mix = self.blocks[i].resid_mix.to(dtype=x.dtype)
                x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
                x_out = x_in + self.blocks[i].attn_scale.to(dtype=x_in.dtype)[None, None, :] * self.ssm_modules[i](self.blocks[i].attn_norm(x_in) * self.blocks[i].ln_scale_factor)
                x_out = x_out + self.blocks[i].mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.blocks[i].mlp(self.blocks[i].mlp_norm(x_out) * self.blocks[i].ln_scale_factor, self.mlp_up_bank[i], self.mlp_down_bank[i])
                x = x_out
                raw_v = None
                if v0 is None:
                    v0 = None  # no value embedding from SSM layers
                skips.append(x)
            else:
