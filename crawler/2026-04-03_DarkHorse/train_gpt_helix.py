...
class Hyperparameters():
    ...
    helix_dim = int(os.environ.get("HELIX_DIM", 64))
    helix_stride = int(os.environ.get("HELIX_STRIDE", 1))
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        # Helix: dual-stream co-firing
        self.helix_enabled = h.helix_enabled
        self.helix_stride = h.helix_stride
        if self.helix_enabled:
            hdim = h.helix_dim
            mdim = h.model_dim
            # Shared crawler block (fires alongside flat layers)
            self.crawler_block = Block(mdim, h.num_heads, h.num_kv_heads, h.mlp_mult,
                                       h.rope_base, h.qk_gain_init, h.train_seq_len,
                                       layer_idx=h.num_layers, ln_scale=h.ln_scale)
            if h.rope_dims > 0:
                head_dim = mdim // h.num_heads
                self.crawler_block.attn.rope_dims = h.rope_dims
                self.crawler_block.attn.rotary = Rotary(head_dim, base=h.rope_base,
                    train_seq_len=h.train_seq_len, rope_dims=h.rope_dims)
            # Cross-injection: flat↔crawler (linear projection, position-agnostic)
            self.helix_f2c_down = nn.Linear(mdim, hdim, bias=False)
            self.helix_f2c_up = nn.Linear(hdim, mdim, bias=False)
            self.helix_c2f_down = nn.Linear(mdim, hdim, bias=False)
            self.helix_c2f_up = nn.Linear(hdim, mdim, bias=False)
            self.helix_merge_gate = nn.Parameter(torch.zeros(mdim))
            # Zero-init up projections for warm start
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        # Helix: initialize crawler stream
        x_crawl = x.clone() if self.helix_enabled else None
        step_count = 0

        def _helix_step(x_flat, x_crawl, x0):
            """Bidirectional cross-injection + crawler fire."""
            if x_crawl is None:
                return x_flat, None
            ...
            x_crawl = self.crawler_block(x_crawl, x0)
            # Cross-inject: crawler → flat
            flat_hint = self.helix_c2f_up(self.helix_c2f_down(x_crawl))
            x_flat = x_flat + flat_hint
            return x_flat, x_crawl
        ...
        for vi in range(num_enc):
            ...
            step_count += 1
            if self.helix_enabled and step_count % self.helix_stride == 0:
                x, x_crawl = _helix_step(x, x_crawl, x0)
            skips.append(x)
        ...
        for vi in range(num_dec):
            ...
            if self.helix_enabled and step_count % self.helix_stride == 0:
                if is_parallel_mode:
                    # In parallel mode, inject into the merged signal
                    x_merged = self.lane_merge.to(dtype=lane0.dtype) * lane0 + (1 - self.lane_merge.to(dtype=lane0.dtype)) * lane1
                    x_merged, x_crawl = _helix_step(x_merged, x_crawl, x0)
                    # Feed back into both lanes
                    delta = x_merged - (self.lane_merge.to(dtype=lane0.dtype) * lane0 + (1 - self.lane_merge.to(dtype=lane0.dtype)) * lane1)
                    lane0 = lane0 + delta
                    lane1 = lane1 + delta
                else:
                    x, x_crawl = _helix_step(x, x_crawl, x0)
        ...
        # Merge helix crawler stream
        if self.helix_enabled and x_crawl is not None:
...
def train_model(h: Hyperparameters, device: torch.device, val_data: ValidationData) -> None:
    ...
    if h.helix_enabled:
        log(f"helix:enabled dim={h.helix_dim} stride={h.helix_stride}")
