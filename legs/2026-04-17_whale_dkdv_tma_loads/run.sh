#!/usr/bin/env bash
# Bench the whale dkdv kernel with vs without TMA loads on the same 4 shapes
# as Lever A. The vault patch in vault_patch.md must be applied first (adds
# the new `_attn_bwd_dkdv_tma_loads_kernel` next to the existing
# `_attn_bwd_dkdv_kernel`; gated by WHALE_BWD_KV_TMA_LOADS).
#
# Usage:
#   # with the vault patch from vault_patch.md already in place:
#   bash legs/2026-04-17_whale_dkdv_tma_loads/run.sh before  # env=0, original kernel
#   bash legs/2026-04-17_whale_dkdv_tma_loads/run.sh after   # env=1, TMA-loads kernel
#
# Outputs land in evidence/ and logs/ under this leg directory.
set -euo pipefail

REPO_ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)"
LEG_DIR="${REPO_ROOT}/legs/2026-04-17_whale_dkdv_tma_loads"
...
PHASE="${1:-before}"
case "${PHASE}" in
  before) export WHALE_BWD_KV_TMA_LOADS=0 ;;
  after)  export WHALE_BWD_KV_TMA_LOADS=1 ;;
...
echo "[run] phase=${PHASE} ts=${TS} WHALE_BWD_KV_TMA_LOADS=${WHALE_BWD_KV_TMA_LOADS}" | tee "${LOG_FILE}"
...
PYTHONPATH="${REPO_ROOT}" ${PY} \
  ...
  --out "${NUMERICS_OUT}" \
  --label "dkdv_tma_loads_${PHASE}_${TS}" \
  2>&1 | tee -a "${LOG_FILE}"

# Per-shape backend bench: same 4 shapes as Lever A.
