...
TRAIN_SCRIPT="${REPO_ROOT}/legs/2026-04-13_midnight_iii_v_2_postema_gptq/train_gpt.py"
TRACKED_ENV="${REPO_ROOT}/legs/2026-04-13_midnight_iii_v_2_postema_gptq/tracked_env.sh"
LOG_DIR="${REPO_ROOT}/legs/2026-04-13_midnight_iii_v_2_postema_gptq/logs"
mkdir -p "${LOG_DIR}"
...
# LEG_DIFF_GUARD_ARGS="--max-code-changes 80 --max-total-changed-lines 120" bash ${REPO_ROOT}/legs/2026-04-13_midnight_iii_v_2_postema_gptq/gate.sh
DEFAULT_LEG_DIFF_GUARD_ARGS="--max-code-changes 300 --max-total-changed-lines 400"
if [[ -n "${LEG_DIFF_GUARD_ARGS:-}" ]]; then
  # shellcheck disable=SC2086
  python3 "${REPO_ROOT}/scripts/leg_diff_guard.py" "${REPO_ROOT}/legs/2026-04-13_midnight_iii_v_2_postema_gptq" ${LEG_DIFF_GUARD_ARGS}
else
  # shellcheck disable=SC2086
  python3 "${REPO_ROOT}/scripts/leg_diff_guard.py" "${REPO_ROOT}/legs/2026-04-13_midnight_iii_v_2_postema_gptq" ${DEFAULT_LEG_DIFF_GUARD_ARGS}
fi
