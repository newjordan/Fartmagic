...
ARM_FILE="${ROOT_DIR}/04_ssm_e2e_ttt_long_context/arms/22_v1_2_longctx_ttt_densehash_non_ngram_brotli.env"
if [[ ! -f "${ARM_FILE}" ]]; then
...
RUNS_SUBDIR="${RUNS_SUBDIR:-v1_2_submission_non_ngram_brotli}"
SUBMISSION_PROFILE="${SUBMISSION_PROFILE:-track_10min_16mb}"
OUT_DIR="${ROOT_DIR}/runs/${RUNS_SUBDIR}/${TS}_s${SEED}"
mkdir -p "${OUT_DIR}"
LOG_PATH="${OUT_DIR}/v1_2_longctx_ttt_densehash_non_ngram_brotli.log"
...
echo "============================================================"
echo "V1.2 submission test (non-ngram + brotli)"
...
if (
  ...
  export RUN_ID="v1_2_non_ngram_brotli_04_longctx_ttt_densehash_s${SEED}_${TS}"
...
if ! grep -q 'Serialized model mixed+brotli:' "${LOG_PATH}"; then
  echo "compression_check_failed: expected brotli artifact path not detected" >&2
  status="fail_compressor"
fi
...
printf "lane\tarm\tstatus\tcompressor\tmodel_params\tdiag_bpb\tsw_bpb\ttotal_size_mixed_bytes\ttotal_size_int6_bytes\tstep_avg_ms\tlog\n" > "${SUMMARY}"
printf "v1_2_non_ngram_brotli\t22_v1_2_longctx_ttt_densehash_non_ngram_brotli\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
  "${status}" "${compressor}" "${model_params}" "${diag_bpb}" "${sw_bpb}" "${size_mixed}" "${size_int6}" "${step_avg_ms}" "${LOG_PATH}" \
  >> "${SUMMARY}"

echo
echo "v1_2_submission_test_complete"
