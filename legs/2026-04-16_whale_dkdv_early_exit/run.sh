#!/usr/bin/env bash
# Bench the whale dkdv kernel before/after the SeparateMaskingIterations
# patch on the same 4 shapes as Lever A. This script does NOT modify
# vault/whale_kernel_triton.py — it must be patched by hand per
# vault_patch.md before the AFTER pass.
#
# Usage:
#   bash legs/2026-04-16_whale_dkdv_early_exit/run.sh before
#   # (apply vault_patch.md to vault/whale_kernel_triton.py)
#   bash legs/2026-04-16_whale_dkdv_early_exit/run.sh after
#
# Outputs land in evidence/ and logs/ under this leg directory.
set -euo pipefail

REPO_ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)"
LEG_DIR="${REPO_ROOT}/legs/2026-04-16_whale_dkdv_early_exit"
...
PYTHONPATH="${REPO_ROOT}" ${PY} \
  ...
  --out "${NUMERICS_OUT}" \
  --label "dkdv_early_exit_${PHASE}_${TS}" \
  2>&1 | tee -a "${LOG_FILE}"
