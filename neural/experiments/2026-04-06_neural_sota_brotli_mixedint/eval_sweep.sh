...
# Use saved checkpoint from brotli+mixedint training run
export INIT_MODEL_PATH="${INIT_MODEL_PATH:-$ROOT/final_model_brotli_mixedint_s444.pt}"
if [[ ! -f "$INIT_MODEL_PATH" ]]; then
    echo "FATAL: checkpoint not found at $INIT_MODEL_PATH"
    echo "Run: cp final_model.pt final_model_brotli_mixedint_s444.pt"
    exit 1
fi
...
# Arm A: control — naive mixed-int (what we already ran, for comparison)
run_arm "A_naive_mix" 1 5 6 8

# Arm B: GPTQ + mixed-int (primary bet)
run_arm "B_gptq_mix" 0 5 6 8

# Arm C: GPTQ + all-int6 + brotli (safety check)
run_arm "C_gptq_int6" 0 6 6 8

# Arm D: GPTQ + aggressive int5 everywhere
run_arm "D_gptq_int5" 0 5 5 8
